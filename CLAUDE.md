...
## Visit OS secure realtime handoff — 2026-09-19 (updated same day, see below)

The generic `server/src/realtime.ts` room relay remains preserved for existing Consult/WebRTC behavior and is not the secure Visit OS transport. `server/src/visitRealtimePolicy.ts` defines the fail-closed authorization boundary: authentication alone is insufficient; the exact patient/clinician membership, visit id, role, visit window, and bounded signaling envelope must all agree.

**This blocker is resolved — do not re-read it as still open.** Landed the same day, later on main:
- `server/src/store.ts` (`VisitMembership`) is the canonical server-side registry resolving `visitId -> exact patientUserId + clinicianUserId + status/window`; `server/src/visits.ts` exposes it as `GET/POST /api/visits` + activate/end transitions, doctor-only, audited.
- `server/src/realtime.ts` now authenticates the WebSocket, resolves the membership from `visitId` alone (never a client-supplied shape), authorizes via `authorizeVisitRealtimeJoin`, isolates the `visit:*` room namespace from the legacy generic relay (`isReservedVisitRealtimeRoom`), validates every `visit-rtc-*` signal (`validateVisitRealtimeSignalEnvelope`) and enforces per-stream replay protection (`createVisitRealtimeReplayGuard`) before relaying, with audit events on join/leave/reject.
- `scripts/uji/visit-realtime-policy.mts` (added by this session) is the first deterministic test of `visitRealtimePolicy.ts` itself — authorization matrix, envelope validation, replay guard, room isolation — plus source-level assertions that `realtime.ts` actually wires each piece in.

**The real remaining blocker, verified by reading the current client code, not assumed:** nothing in `src/` uses `visit-join`/`visit-rtc-*` yet. `src/components/VisitCommandCenter.tsx` still builds `ConsultChat`'s `room` from `'visit-' + safeToken(recordId || activePatient.id)` — a client-derived room name over the legacy generic relay — and never calls the new `/api/visits` API or creates a `VisitMembership`. That is not an oversight to just "wire up": the AI-EMR `Patient` record (`src/lib/types.ts`) that `activePatient` points at has **no email or account-linkage field at all**, while `VisitMembership.patientUserId` must be a real platform `User.id` (role `pasien`). There is currently no data path from a clinical patient chart to an authenticated patient account. `Consult.patientEmail` (`src/lib/types.ts`) is the closest existing concept, but nothing links a `Consult` to a `Patient` record today, and `VisitCommandCenter`'s existing `patientVisit` lookup deliberately compares against the *doctor's own* email (a safe non-match, not a bug to "fix" by weakening it).

Do not paper over this by inventing a synthetic `patientUserId` (e.g. reusing the clinical patient id) — that defeats the whole authorization boundary this handoff exists to build. The next real step is a product decision plus a small explicit UI flow: let a doctor deliberately link an active clinical patient to a registered patient account (e.g. via the existing authenticated `/api/users/search`), persist that link, and only then let `VisitCommandCenter` create a `VisitMembership` and switch `ConsultChat`'s signaling to the secure `visit:` channel for that specific patient — falling back to the current generic relay unchanged whenever no such link exists. Preserve existing Consult compatibility throughout.
