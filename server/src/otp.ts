...
//
// Nomor telepon masih dipakai di Connect hanya untuk mendeteksi akun ganda
// lewat sidik yang tidak dapat dikembalikan; OTP login sendiri hanya lewat email.
...
export async function emailOtpStart(req: Request, res: Response) {
  ...
  if (!sent) {
    // A code that never left the server must never remain verifiable, and the
    // user should be able to retry immediately after a transient mail failure.
...
export async function emailOtpVerify(req: Request, res: Response) {
  ...
  // One successful verification consumes the code before any user/session work.
  emailCodes.delete(email)
