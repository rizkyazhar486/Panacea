...
await denganFetchPalsu(async (input, init) => {
  ...
  ok('hasil studi dinormalisasi menjadi satu record', hasil.length === 1)
  ok('NCT ID dipertahankan', hasil[0]?.nctId === 'NCT01234567')
  ...
  ok('tautan selalu kembali ke registry asli', hasil[0]?.url === 'https://clinicaltrials.gov/study/NCT01234567')
})
