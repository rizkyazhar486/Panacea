...
// Out-of-range or malformed environmental numbers must be dropped, not surfaced as facts.
{
  const fakeFetch: typeof fetch = async (input) => {
    const url = String(input)
    if (url.includes('air-quality-api.open-meteo.com')) {
      return jsonResponse({ current: { european_aqi: -1, pm2_5: 'NaN', pm10: 99999 } })
    }
    return jsonResponse({
      current: { uv_index: 99, temperature_2m: 999, apparent_temperature: -999, relative_humidity_2m: 101 },
      daily: { uv_index_max: [44], sunrise: ['x'.repeat(200)], sunset: [''] },
    })
  }
  const result = await lingkunganKota('Jakarta', fakeFetch)
  assert.equal(result.aqi, undefined)
  assert.equal(result.pm25, undefined)
  assert.equal(result.pm10, undefined)
  assert.equal(result.uv, undefined)
  assert.equal(result.suhuC, undefined)
  assert.equal(result.terasaC, undefined)
  assert.equal(result.lembapPct, undefined)
  assert.equal(result.error, 'tidak_terjawab')
}
