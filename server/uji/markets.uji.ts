...
try {
  // Quote normalization: keep the delayed/source disclosure, skip null market
  // gaps, convert Unix seconds to milliseconds, and preserve a bounded timeout.
  {
    let requestedUrl = ''
  ...
  // Search must treat user input as data, filter symbols that the quote endpoint
  // cannot accept, and map known Yahoo quote types to UI labels.
  {
    let requestedUrl = ''
  ...
  // Per-symbol failures must not erase successful watchlist members.
  {
    globalThis.fetch = (async (input) => {
