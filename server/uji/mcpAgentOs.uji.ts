...
assert.ok(evidence.rationale.some((item) => item.code === 'independent_verification'))
...
const requestedWrite = routeAgentTask({
  intent: 'feature',
  capabilities,
  requestedPermissionTier: 'scoped-reversible-write',
})
assert.equal(requestedWrite.permissionTier, 'scoped-reversible-write')
assert.equal(requestedWrite.requiresExplicitAuthorization, false)
