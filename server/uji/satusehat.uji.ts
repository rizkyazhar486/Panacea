...
try {
  // Deterministic bundle generation: valid RFC 4122 UUID URNs, no fabricated
  // gender, bounded/filtered clinical content and stable timestamps.
  {
    const uuids = [
    ...
    assert.equal(bundle.entry.length, 4, 'Patient + Encounter + one Condition + one valid Observation')
    ...
    assert.equal(patient.resource.gender, 'unknown', 'unknown sex must not be fabricated as male/female')
    assert.equal(patient.resource.birthDate, undefined, 'invalid calendar date must be omitted')
  ...
  // Path/resource validation happens before OAuth/network I/O.
  {
    let calls = 0
  ...
  // OAuth request is bounded and malformed successful payloads fail closed.
  {
  ...
  // Canonical POST performs OAuth then FHIR request; secret remains in form body,
  // never in the URL, and the outbound resource retains its declared type.
  {
  ...
  // Unconfigured mode is preview-only and must not touch the network.
  {
