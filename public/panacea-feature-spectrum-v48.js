(() => {
  ...
  const PALETTES = {
    astralChrome: { id: 'astral-chrome', name: 'Astral Chrome', base: '#181512', accents: ['#D9DEE2', '#22517A', '#917E66', '#A09B98'], metal: '#D9DEE2' },
    auroraSky: { id: 'aurora-sky', name: 'Aurora Sky', base: '#295489', accents: ['#84B4CF', '#F4B183', '#DC8773', '#A895B3'], metal: '#EEC2AC' },
    emberSingularity: { id: 'ember-singularity', name: 'Ember Singularity', base: '#120F12', accents: ['#9D7258', '#6C4539', '#D4C4B4', '#3B2A26'], metal: '#D4C4B4' },
    crimsonWeb: { id: 'crimson-web', name: 'Crimson Web', base: '#1A1819', accents: ['#AD3F34', '#881416', '#12363F', '#9D9990'], metal: '#9D9990' },
    imperialIvory: { id: 'imperial-ivory', name: 'Imperial Ivory', base: '#25221D', accents: ['#E9E5DE', '#A09281', '#92816B', '#DACBB5'], metal: '#E9E5DE' },
    glacialChrome: { id: 'glacial-chrome', name: 'Glacial Chrome', base: '#345161', accents: ['#D7E0E2', '#9CAEB6', '#6C818C', '#C1CBD0'], metal: '#D7E0E2' },
    shadowTeal: { id: 'shadow-teal', name: 'Shadow Teal', base: '#101513', accents: ['#4B756F', '#2B5B5A', '#43C8B9', '#183533'], metal: '#8ED8CE' },
    prismaticSentinel: { id: 'prismatic-sentinel', name: 'Prismatic Sentinel', base: '#100C14', accents: ['#DBE5DD', '#335CA1', '#8B191D', '#808595'], metal: '#F4F7F6' },
    noirHalo: { id: 'noir-halo', name: 'Noir Halo', base: '#010101', accents: ['#C7A85B', '#C3C1B9', '#88877F', '#383832'], metal: '#D6C37A' },
    cosmicViolet: { id: 'cosmic-violet', name: 'Cosmic Violet', base: '#161436', accents: ['#835B92', '#453A70', '#AF77A6', '#E6A9AE'], metal: '#D3B0C5' },
  }

  const GROUP_RULES = [
    { palette: PALETTES.cosmicViolet, test: /(body[- ]?explorer|genome|frontier|discovery|invention|innovation|anatom|atlas|molecular|dna|organ|body exposure)/i },
    { palette: PALETTES.shadowTeal, test: /(nutrition|longevity|wellness|biological[- ]?age|life[- ]?compass|mind|meditation|habit|metabolic|healthy aging)/i },
    { palette: PALETTES.glacialChrome, test: /(recovery|sleep|radiology|health[- ]?data|your numbers|body signals|vitapulse|imaging|ct|mri|ultrasound)/i },
    { palette: PALETTES.emberSingularity, test: /(emergency|sos|urgent|critical|crisis|trauma|resusc|warning|alert)/i },
    { palette: PALETTES.crimsonWeb, test: /(clinical|drug|calculator|labs?|evidence|electrophysiology|arrhythm|heart|cardio|medical records?|emr|diagnos|triage)/i },
    { palette: PALETTES.imperialIvory, test: /(med[- ]?study|knowledge|education|osce|ukmppd|library|tutorial|curriculum|learn|look up|study)/i },
    { palette: PALETTES.auroraSky, test: /(latihan|training|workout|fitness|move|run|sport|exercise|performance|vo2|max aerobic)/i },
    { palette: PALETTES.prismaticSentinel, test: /(chat|message|assistant|copilot|ai[- ]|ai |ask panacea|agent|conversation)/i },
    { palette: PALETTES.noirHalo, test: /(settings|profile|account|admin|owner|verification|legal|security|privacy|finance|keuangan|wallet|manage|billing)/i },
  ]
  ...
  function choosePalette(identity) {
    ...
    for (const rule of GROUP_RULES) if (rule.test.test(source)) return rule.palette
  ...
  function tokensFor(palette, identity) {
    ...
    return {
      palette, primary, secondary, tertiary, metal: palette.metal,
      soft: rgba(primary, 0.12), softStrong: rgba(primary, 0.19), border: rgba(primary, 0.32), focus: rgba(primary, 0.72),
      glow: rgba(primary, 0.22), glowStrong: rgba(primary, 0.34), secondarySoft: rgba(secondary, 0.13), metalSoft: rgba(palette.metal, 0.14),
    }
  }
  ...
  function paintAssistive() {
    const button = document.querySelector('button[aria-label="Buka menu navigasi"], button[aria-label="Tutup menu"]')
    if (button) {
