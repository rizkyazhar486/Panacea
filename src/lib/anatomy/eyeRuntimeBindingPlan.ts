...
export interface EyeRuntimeLoadedSource { readonly file: string }
...
function selectedCandidate(result: AtlasMeshBindingResult, meshNodeId: string): AtlasMeshBindingCandidate | null {
...
export function buildEyeRuntimeBindingPlan(
  ...
  for (const meshNodeId of binding.selectedMeshNodeIds) {
    if (!meshNodeId.trim() || seenMeshIds.has(meshNodeId)) { blockers.push(`invalid-selected-mesh:${meshNodeId}`); continue }
    seenMeshIds.add(meshNodeId)
    const candidate = selectedCandidate(binding, meshNodeId)
    if (!candidate) { blockers.push(`selected-mesh-candidate-not-unique:${meshNodeId}`); continue }
    const identityKind = exactIdentityKind(candidate)
    if (!identityKind) { blockers.push(`selected-mesh-not-exact-identity:${meshNodeId}`); continue }
    if (!candidate.sourceFile.trim() || !loadedFiles.has(candidate.sourceFile)) { blockers.push(`source-file-not-runtime-loaded:${candidate.sourceFile || meshNodeId}`); continue }
    if (!candidate.sourceName.trim()) { blockers.push(`missing-source-name:${meshNodeId}`); continue }
    targets.push({ canonicalNodeId: key, meshNodeId, sourceName: candidate.sourceName, sourceFile: candidate.sourceFile, exactIdentityKind: identityKind })
  }
  ...
  return {
    status, blockers, canonicalNodeId: key, targets: status === 'eligible' ? targets : [],
    mayTraverseObject3D: false, mayLoadAsset: false, mayMutateGeometry: false,
    mayInferAnatomy: false, mayPromoteAcademicReview: false, patientSpecific: false,
  }
}
