...
export interface EyeRendererCanonicalTarget {
  ...
  /** Optional exact source identity already proven by Panacea-owned provenance. */
...
/**
 * Convert an already-approved multiscale transition into a renderer-neutral
 * instruction. Resolution is exact and injected: this layer never performs
 * fuzzy/source-name fallback, asset loading, Object3D mutation, or camera math.
 */
export function buildEyeRendererViewInstruction(
  ...
  const motion =
    view.motion === 'representational' && transition.mayAnimate
      ? 'representational'
      : 'none'

  if (view.motion === 'representational' && motion !== 'representational') {
    blockers.push('representational-motion-not-authorized')
  }
