...
export function auditObstetricPelvisSourceAcceptance() {
  ...
  const zeroGeometry = OBSTETRIC_PELVIS_CANONICAL_STRUCTURES
  ...
  const fabricatedFemaleReproductive = FEMALE_REPRODUCTIVE_NAMES_NOT_SHIPPED_IN_OBSTETRI.filter((name) =>
    byName.has(name),
  )
  ...
  return {
    ...
    sourceOnly,
    missingCanonical,
    zeroGeometry: zeroGeometry.map((part) => part.name),
    fabricatedFemaleReproductive,
    blockers,
    renderEligible:
      Boolean(info) &&
      parts.length > 0 &&
      ...
      fabricatedFemaleReproductive.length === 0 &&
      blockers.length === 0,
  }
}
