...
/**
 * These are exact identities from the generated shipped specialty atlas. They are
 * intentionally module-scoped: a cochlea cannot be satisfied by an external ear,
 * a cornea cannot be satisfied by another ocular surface, and no left/right mesh is
 * mirrored to stand in for its counterpart.
 */
export const SENSORY_ENT_TARGETS: readonly SensoryEntTarget[] = [
  {
    domain: 'upper-airway-ent',
    module: 'tht',
    label: 'Nose, pharynx and larynx',
    expectedSource: 'bodyparts3d',
    requiredExactNames: [
      'Tongue',
      'Epiglottis',
      'External ear',
  ...
  },
  {
    domain: 'eye',
    module: 'mata',
    label: 'Eye, orbit and optic pathway',
    expectedSource: 'bodyparts3d',
    requiredExactNames: [
      'Left cornea',
      'Right cornea',
...
export function auditSensoryEntTarget(target: SensoryEntTarget): SensoryEntAudit {
  ...
  const sourceBacked = Boolean(info)
    ...
    && sources.includes(target.expectedSource)
...
export function auditSensoryEntBreadth(): SensoryEntAudit[] {
  return SENSORY_ENT_TARGETS.map(auditSensoryEntTarget)
}

export function sensoryEntRenderEligible(audit: SensoryEntAudit): boolean {
  return audit.reachable && audit.sourceBacked && audit.blockers.length === 0
}

export const SENSORY_ENT_RENDER_FORMULA =
  'SensoryRenderEligible = ReachableModule ∧ ExactNamedGeometry ∧ PositiveIndexedTriangles ∧ ExpectedSourceIdentity' as const
