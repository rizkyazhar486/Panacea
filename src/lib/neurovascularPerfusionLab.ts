...
export interface NeurovascularRelationship { id: string; expression: string; meaning: string; boundary: string }
export interface NeurovascularNetworkNode { id: string; label: string; layer: 'systemic' | 'intracranial' | 'vascular' | 'delivery' | 'metabolic'; role: string }
export interface NeurovascularEvidenceAnchor { pmid: string; title: string; role: string; url: string }
...
export const NEUROVASCULAR_NETWORK: readonly NeurovascularNetworkNode[] = [
  { id: 'systemic-pressure', label: 'Systemic pressure drive', layer: 'systemic', role: 'Provides the upstream pressure component that contributes to the cerebral perfusion gradient.' },
  { id: 'intracranial-pressure-context', label: 'Intracranial pressure / volume context', layer: 'intracranial', role: 'Represents downstream pressure and compliance context created by interactions among brain tissue, blood and CSF volumes.' },
  { id: 'autoregulation', label: 'Autoregulatory vascular tone', layer: 'vascular', role: 'Adjusts cerebrovascular resistance across changing perfusion pressure to buffer cerebral blood flow within physiologic reserve.' },
  { id: 'cerebral-flow', label: 'Cerebral blood flow', layer: 'delivery', role: 'Carries oxygen and metabolic substrate from arterial circulation into the neurovascular unit.' },
  { id: 'oxygen-delivery', label: 'Oxygen delivery', layer: 'delivery', role: 'Couples flow to arterial oxygen content before tissue extraction and cellular utilization.' },
  { id: 'metabolic-demand', label: 'Neural metabolic demand', layer: 'metabolic', role: 'Represents synthetic tissue demand that must remain matched to substrate and oxygen delivery for homeostasis.' },
] as const
