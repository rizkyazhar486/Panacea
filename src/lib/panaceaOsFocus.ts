...
/**
 * Resolve a deliberately small OS focus rail from explicit user-entered items.
 *
 * No health score, readiness estimate, diagnosis, calendar inference, or model
 * output is generated here. The resolver only orders unfinished items by their
...
export function resolvePanaceaOsFocus(items: PanaceaOsItem[], nowMinutes: number): PanaceaOsFocusSlot[] {
  ...
  return pending.slice(0, 3).map((item, index) => ({
    ...
    item,
    ...classifyTiming(item, nowMinutes),
  }))
}
