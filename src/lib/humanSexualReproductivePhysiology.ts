...
export function stagesForDomain(domain: SexualReproductiveDomain): PhysiologyStage[] {
  return HUMAN_SEXUAL_REPRODUCTIVE_PHYSIOLOGY.filter((stage) => stage.domain === domain).sort((a, b) => a.sequence - b.sequence)
}

export function stageAt(domain: SexualReproductiveDomain, normalizedProgress: number): PhysiologyStage | null {
