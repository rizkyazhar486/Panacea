...
/**
 * Urutkan tab supaya tiap kelompok menjadi satu ketetanggaan bersambung.
 *
 * Stabil di dalam kelompok: urutan asli dipertahankan, jadi hanya keanggotaan
 * kelompok yang menentukan perpindahan. Tab yang belum dipetakan berkumpul di
 * kelompok terakhir dan TETAP ADA -- mengurutkan tidak boleh menjadi cara lain
 * untuk menjatuhkan sebuah tab.
 */
export function urutkanMenurutKelompok<T>(tab: readonly T[], kunci: (t: T) => string): T[] {
  const peringkat = (t: T) => {
    const i = URUTAN_KELOMPOK.indexOf(kelompokUntuk(kunci(t)))
    return i < 0 ? URUTAN_KELOMPOK.length : i
  }
  return tab
    .map((t, i) => ({ t, i, r: peringkat(t) }))
