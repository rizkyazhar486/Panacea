...
export function privacyTrimRoute<T extends { lat: number; lng: number }>(points: T[], bufferM = 200): T[] {
  if (points.length < 3 || bufferM <= 0) return points
  let fromStart = 0
  let startIndex = 0
  for (let i = 1; i < points.length; i++) {
    fromStart += haversineM(points[i - 1], points[i])
    if (fromStart >= bufferM) { startIndex = i; break }
  }
  let fromEnd = 0
  let endIndex = points.length - 1
  for (let i = points.length - 2; i >= 0; i--) {
    fromEnd += haversineM(points[i + 1], points[i])
    if (fromEnd >= bufferM) { endIndex = i; break }
  }
  // Short activities remain route-private rather than leaking home/work by
  // pretending a privacy buffer was applied when it could not be.
