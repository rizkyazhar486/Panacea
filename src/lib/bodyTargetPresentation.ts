...
/**
 * Compile renderer-neutral trust state into presentation-safe UI metadata.
 *
 * This function never upgrades evidence. A target can only receive
 * `verified-anatomy` when the underlying BodyTargetTrust gate permits it.
 * Reference-only and verification-required targets remain visibly reference
 * content. Blocked or not-represented targets are hidden rather than being
 * substituted with invented geometry.
 */
