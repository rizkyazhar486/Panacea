...
/**
 * Extract one spatial record per named source node in a single bottom-up pass.
 *
 * Every mesh bounding box is transformed to source/world space exactly once,
 * then child bounds are unioned upward. A named parent therefore receives the
 * union of its actual descendant geometry without repeatedly traversing the
 * same subtree with `Box3.setFromObject()`.
 *
 * Complexity is O(objects + meshes) apart from sorting the final records.
 */
...
/**
 * Return source structures whose axis-aligned bounding volumes intersect a
 * plane/slab. This is a spatial candidate query, not a claim that the exact
 * triangle mesh intersects the plane; consumers that need pixel-perfect
 * section contours must clip/raycast the actual geometry after this pruning.
 */
