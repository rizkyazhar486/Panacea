...
export function evaluateProjectionReadiness(
  ...
  const claimsHumanReview = provenance.evidenceStatus === 'human-reviewed' || provenance.academicReview === 'recorded'
  const reviewRequired = target.academicReview === 'recorded' || claimsHumanReview
