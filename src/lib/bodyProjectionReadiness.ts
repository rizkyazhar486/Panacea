 ...
 * preserves transformation history, and both target + asset carry a recorded
 * academic review with qualified reviewer metadata. Pending review may remain
 * a reference/verification-required state but must never become verified.
 */
export function evaluateProjectionReadiness(
  ...
  if (target.academicReview !== 'recorded') {
    reasons.push('Verified anatomy requires recorded target academic review.')
  }
  if (provenance.academicReview !== 'recorded') {
    reasons.push('Verified anatomy requires recorded asset academic review.')
  } else {
