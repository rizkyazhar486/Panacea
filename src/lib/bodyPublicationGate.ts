  ...
  if (request.mode === 'evidence-overlay') {
    ...
    for (const [index, record] of evidence.entries()) {
      ...
      for (const reason of validation.reasons) {
        ...
        reasons.push(mapped)
        // This one validator reason is publication-only. Missing/malformed source,
        // locator, anatomy mapping, patient boundary, or reviewer metadata remains
        // a hard reference-display failure.
