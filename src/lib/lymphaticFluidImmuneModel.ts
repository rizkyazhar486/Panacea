export type LymphaticTeachingInputs = {
  filtrationLoad: number
  collectingPump: number
  skeletalMotion: number
  nodalTransit: number
}

export type LymphaticTeachingState = {
  returnCapacity: number
  interstitialBurden: number
  immuneTransit: number
}

const clamp01 = (value: number) => Math.max(0, Math.min(1, Number.isFinite(value) ? value : 0))

/**
 * Synthetic, dimensionless educational relationships only.
 * These terms are not measurements, clinical equations, diagnostic probabilities, or treatment targets.
 */
...
export const LYMPHATIC_EDUCATION_MODEL = {
  ...
  evidence: [
    {
      pmid: '9419570',
      role: 'Background review describing lymph transport, tissue-fluid/macromolecule return, propulsion, and impaired immune-cell trafficking in lymphatic failure.',
      source: 'PubMed',
    },
    {
      pmid: '24503053',
      role: 'Review describing lymphatic fluid drainage and immune-cell trafficking in a tissue-specific context.',
      source: 'PubMed',
    },
  ],
  provenance: {
    state: 'reference-and-simulated',
    modelVersion: '1.0.0',
    evidenceCheckedOn: '2026-09-18',
  },
