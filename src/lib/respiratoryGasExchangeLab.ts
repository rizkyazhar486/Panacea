...
export function deriveRespiratoryGasExchange(inputsLike: Partial<RespiratoryGasExchangeInputs> = {}): RespiratoryGasExchangeOutputs {
  ...
  const alveolarVentilationSignal = clamp01(
    0.12 + inputs.ventilationDrive * 0.52 + complianceEfficiency * 0.20 + airwayPatency * 0.20 - airwayResistanceSignal * 0.10,
  )
  ...
  const workOfBreathingSignal = clamp01(0.50 * airwayResistanceSignal + 0.32 * (1 - inputs.compliance) + 0.18 * inputs.ventilationDrive)
  const gasExchangeReserveSignal = clamp01(0.10 + oxygenTransferSignal * 0.52 + co2ClearanceSignal * 0.28 + inputs.compliance * 0.10 - inputs.metabolicDemand * 0.44)
