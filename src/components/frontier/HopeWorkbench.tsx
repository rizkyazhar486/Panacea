...
function MentalWorkbench() {
  return (
    <div className="rounded-3xl border border-brand/20 bg-brand/[0.04] p-4">
      ...
      <h4 className="mt-1 text-base font-black text-ink dark:text-white">A plan that exists before a crisis is easier to use during one.</h4>
      <p className="mt-2 text-xs leading-relaxed text-neutral-500">
        The Mind hub now contains a local-first seven-step safety plan. It is stored on this device, is not monitored,
        does not calculate suicide risk, and keeps emergency disposition with humans and local services.
      </p>
      <Link to="/mind" className="mt-3 inline-flex min-h-11 items-center rounded-full bg-brand px-4 text-xs font-black text-white">
        Open Mind → Safety plan
      </Link>
    </div>
  )
}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <Metric label="PPV" value={`${(result.ppv * 100).toFixed(1)}%`} note="Chance a positive result is truly positive under these assumptions." />
        <Metric label="NPV" value={`${(result.npv * 100).toFixed(1)}%`} note="Chance a negative result is truly negative under these assumptions." />
        <Metric label="False positives / 10k" value={result.fp.toFixed(0)} />
        <Metric label="Missed cases / 10k" value={result.fn.toFixed(0)} />
      </div>
      <div className="rounded-2xl border border-neutral-200 p-3 font-mono text-[10px] leading-relaxed text-neutral-500 dark:border-white/10">
        PPV = Se×Prev / [Se×Prev + (1−Sp)×(1−Prev)] · expected cohort = 10,000 people
      </div>
...
function PredictionWorkbench() {
  ...
  const result = useMemo(() => {
    const p = probabilities.split(',').map((value) => Number(value.trim())).filter((value) => Number.isFinite(value))
    ...
    const score = p.reduce((sum, value, index) => sum + (value - y[index]) ** 2, 0) / p.length
    return { score, n: p.length }
    ...
    <div className="space-y-3">
      <div className="grid gap-2 md:grid-cols-2">
        <label className="rounded-2xl border border-neutral-200 p-3 dark:border-white/10">
          <span className="text-[9px] font-black uppercase tracking-wide text-neutral-500">Example predicted probabilities (0–1)</span>
      ...
      {result ? (
        <div className="grid gap-2 sm:grid-cols-2">
          <Metric label="Brier score" value={result.score.toFixed(4)} note="Lower is better for the same outcome, horizon and evaluation population." />
          <Metric label="Observations" value={String(result.n)} note="A tiny toy sample is educational only; real validation needs representative external cohorts." />
        </div>
      ) : (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-3 text-xs font-bold text-red-700 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-200">
          Use the same number of probabilities and binary outcomes, with each probability between 0 and 1.
        </div>
      )}
      ...
      <p className="text-[10px] leading-relaxed text-neutral-400">This checks probabilistic error, not clinical usefulness by itself. Calibration, discrimination, fairness, drift and decision impact remain separate requirements.</p>
    </div>
  )
}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        ...
        <Metric label="PV of uncovered annual needs" value={`${result.pv.toFixed(2)} units`} note="Generic units; not a currency quote or financial recommendation." />
      </div>
      ...
      <p className="text-[10px] leading-relaxed text-neutral-400">Scenario planning only. The lifespan value is an assumption, not Panacea predicting how long a person will live.</p>
    </div>
  )
}
...
function InfrastructureWorkbench() {
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-2 sm:grid-cols-2">
        <NumberField label="Population age 65+" value={older} onChange={setOlder} min={0} suffix="people / units" />
        <NumberField label="Population age 20–64" value={working} onChange={setWorking} min={0.1} suffix="people / units" />
      </div>
      <Metric label="Old-age dependency ratio" value={`${ratio.toFixed(1)} per 100`} note="Population age 65+ per 100 people age 20–64. It does not measure disability, productivity or care need by itself." />
...
function GeroscienceWorkbench() {
  return (
    <div className="space-y-2">
      ...
      <p className="text-[10px] leading-relaxed text-neutral-400">Panacea may organize target, biomarker, trial and regulatory evidence. It must not output synthesis recipes, dosing or self-experimentation protocols.</p>
    </div>
  )
}
...
function RegenerativeWorkbench() {
  const rows = [
    ...
    ['Cell therapies', 'Indication-specific', 'Some regulated clinical uses exist; maturity varies by cell product and disease'],
    ['Tissue engineering', 'Preclinical → clinical depending on tissue', 'Must expose scaffold, vascularization, function and immune-compatibility evidence'],
