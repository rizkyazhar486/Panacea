    ...
    <section className="relative isolate overflow-hidden rounded-[32px] border border-white/[.09] bg-[#01040a]/96 p-3 text-white shadow-[0_32px_110px_rgba(0,0,0,.52)] sm:p-5 lg:p-6" aria-label="Panacea command deck">
      ...
      <header className="relative grid gap-5 px-1 pb-6 pt-1 lg:grid-cols-[minmax(0,1fr)_minmax(280px,.72fr)] lg:items-end lg:gap-8 lg:pb-8">
        <div className="min-w-0">
          <div className="mb-3 flex items-center gap-3">
            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-white/15 bg-gradient-to-br from-cyan-200/24 via-violet-300/18 to-fuchsia-300/16 text-xs font-black text-white shadow-[inset_0_1px_0_rgba(255,255,255,.18),0_8px_28px_rgba(34,211,238,.10)]" aria-hidden>{initials}</div>
            <div className="min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(domain !== 'All' || category !== 'All' || query) && <button type="button" onClick={resetFilters} className="rounded-full border border-white/12 bg-white/[.045] px-3 py-1.5 text-[10px] font-black text-white/70 transition hover:border-white/24 hover:text-white">Reset view</button>}
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-[22px] border border-white/[.08] bg-white/[.028] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,.05)] backdrop-blur-xl" aria-label="Capability coverage">
          <div className="mb-2 flex items-center justify-between gap-3"><span className="text-[10px] font-black uppercase tracking-[.15em] text-white/44">Coverage</span><span className="text-[10px] font-black text-white/70">at a glance</span></div>
          <div className="grid grid-cols-4 gap-1.5 sm:grid-cols-6 lg:grid-cols-4">
            {CATEGORY_ORDER.filter((item) => (categoryCounts.get(item) ?? 0) > 0).slice(0, 8).map((item) => {
              ...
              return <button key={item} type="button" onClick={() => { setCategory(item); setDomain('All'); setExpanded(false) }} className="group flex min-w-0 flex-col justify-end rounded-[12px] border border-white/[.055] bg-black/20 p-1.5 text-left transition hover:border-cyan-200/25 hover:bg-white/[.04]" aria-label={`${item}: ${count} capabilities`}><span className="mb-1 block w-full rounded-[7px] bg-gradient-to-t from-cyan-400/55 via-blue-400/35 to-violet-300/55 transition group-hover:brightness-125" style={{ height }} aria-hidden /><span className="truncate text-[8px] font-black text-white/45">{item}</span></button>
            })}
          </div>
        </div>
      </header>
      ...
      <div className="relative grid gap-3 md:grid-cols-3" aria-label="Super pages">
        {DOMAINS.map((item) => {
          ...
          return (
            <button key={item.key} type="button" onClick={() => chooseDomain(item.key)} aria-pressed={selected} className={`group relative min-h-[154px] overflow-hidden rounded-[26px] border p-4 text-left transition duration-300 active:scale-[.985] ${selected ? 'border-white/35 bg-white/[.075] shadow-[0_18px_60px_rgba(0,0,0,.32)]' : 'border-white/[.09] bg-white/[.028] hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[.045]'}`}>
              ...
              <span className="relative flex h-full flex-col justify-between gap-7"><span className="flex items-start justify-between gap-3"><span className="grid h-11 w-11 place-items-center rounded-[15px] border border-white/15 bg-black/22 text-white shadow-[inset_0_1px_0_rgba(255,255,255,.14)]" aria-hidden><DomainIcon size={21} /></span><span className="text-3xl font-black tracking-[-.06em] text-white/90">{count}</span></span><span className="block min-w-0"><span className="flex items-center gap-2"><span className={`h-1.5 w-1.5 rounded-full ${item.accent}`} aria-hidden /><span className="truncate text-lg font-black tracking-tight">{item.short}</span></span><span className="mt-1 block truncate text-[10px] font-bold text-white/48">{item.hint}</span></span></span>
            </button>
          )
        })}
      </div>
      ...
      <div className="relative mt-7">
        <div className="mb-3 flex items-center justify-between gap-3 px-1"><span className="text-[10px] font-black uppercase tracking-[.16em] text-white/46">Quick actions</span><span className="text-[10px] font-bold text-white/30">one tap</span></div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {anchors.map((anchor) => {
            const AnchorIcon = anchor.icon
            return <Link key={anchor.label} to={anchor.to} aria-label={`${anchor.label}: ${anchor.hint}`} className={`group min-h-[112px] overflow-hidden rounded-[22px] border border-white/[.09] bg-gradient-to-br ${anchor.surface} p-3.5 ${anchor.glow} transition duration-200 hover:-translate-y-0.5 hover:border-white/24 active:scale-[.985]`}><div className="flex items-start justify-between gap-2"><span className="grid h-9 w-9 place-items-center rounded-[13px] border border-white/12 bg-black/22 text-white shadow-[inset_0_1px_0_rgba(255,255,255,.12)]" aria-hidden><AnchorIcon size={19} /></span><span className="text-white/36 transition group-hover:translate-x-0.5 group-hover:text-white/72" aria-hidden>↗</span></div><div className="mt-3 truncate text-sm font-black tracking-tight">{anchor.label}</div><div className="mt-0.5 truncate text-[10px] font-semibold text-white/48">{anchor.hint}</div></Link>
          })}
        </div>
      </div>
      ...
      <div className="relative mt-8 rounded-[26px] border border-white/[.075] bg-white/[.022] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,.035)] backdrop-blur-xl sm:p-4">
        <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
          <label className="flex min-h-[48px] min-w-0 flex-1 items-center gap-2.5 rounded-[17px] border border-cyan-200/12 bg-black/30 px-3.5 transition focus-within:border-cyan-200/42 focus-within:bg-black/42"><span className="text-cyan-100/66" aria-hidden>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find any capability…" className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-white outline-none placeholder:text-white/28" />{query && <button type="button" onClick={() => setQuery('')} className="grid h-7 w-7 place-items-center rounded-full text-white/55 transition hover:bg-white/10 hover:text-white" aria-label="Clear search">×</button>}</label>
          <button type="button" onClick={() => setExpanded((value) => !value)} className="min-h-[48px] shrink-0 rounded-[17px] border border-violet-300/15 bg-violet-300/[.07] px-4 text-xs font-black text-violet-50 transition hover:border-violet-200/32 hover:bg-violet-300/[.11]">{expanded ? 'Compact' : `Browse ${filtered.length}`}</button>
        </div>

        <div className="no-scrollbar mt-3 flex gap-2 overflow-x-auto pb-1" aria-label="Feature categories">
          {categories.map((item) => <button key={item} type="button" onClick={() => { setCategory(item); if (item !== 'All') setDomain('All') }} aria-pressed={category === item} className={`min-h-[36px] shrink-0 rounded-full border px-3 text-[10px] font-black transition ${category === item ? 'border-cyan-100/48 bg-cyan-100 text-[#02050a]' : 'border-white/[.075] bg-white/[.028] text-white/58 hover:border-white/18 hover:bg-white/[.045] hover:text-white'}`}>{item}{item !== 'All' ? ` · ${categoryCounts.get(item) ?? 0}` : ''}</button>)}
        </div>

        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          {visible.map((feature) => {
            const featureCategory = categoryOf(feature)
            return <Link key={`${feature.to}-${feature.nama}`} to={canonical(feature.to)} className={`group min-h-[82px] rounded-[18px] border p-3 transition duration-200 hover:-translate-y-px active:scale-[.985] ${featureSurface(featureCategory)}`} aria-label={`${feature.nama} · ${featureCategory}`}><div className="flex items-start justify-between gap-2"><span className="truncate text-[8px] font-black uppercase tracking-[.12em] text-white/38">{featureCategory}</span><span className="text-[10px] text-white/26 transition group-hover:text-white/68" aria-hidden>↗</span></div><div className="mt-3 truncate text-[11px] font-black leading-tight text-white/88">{feature.nama}</div></Link>
          })}
        </div>

        {visible.length === 0 && <div className="mx-auto my-5 grid min-h-[180px] max-w-xl place-items-center rounded-[22px] border border-dashed border-white/10 bg-black/18 p-6 text-center"><div><div className="mx-auto grid h-12 w-12 place-items-center rounded-[16px] border border-cyan-200/14 bg-cyan-300/[.06] text-lg text-cyan-100" aria-hidden>⌕</div><div className="mt-4 text-sm font-black text-white/88">No matching capability</div><button type="button" onClick={resetFilters} className="mt-3 min-h-[40px] rounded-full border border-cyan-100/28 bg-cyan-100 px-4 text-[10px] font-black text-[#02050a] transition hover:brightness-110">Show everything</button></div></div>}

        {!expanded && !query && category === 'All' && domain === 'All' && filtered.length > visible.length && <button type="button" onClick={() => setExpanded(true)} className="mt-3 w-full rounded-[16px] border border-white/[.065] bg-white/[.022] py-3 text-[10px] font-black text-white/52 transition hover:border-cyan-200/24 hover:bg-cyan-200/[.045] hover:text-cyan-50">Show all {uniqueFeatures.length} capabilities</button>}
      </div>
    </section>
  )
}
