          ...
          return (
            <button key={item.key} type="button" onClick={() => chooseDomain(item.key)} aria-pressed={selected} className={`group relative min-h-[154px] overflow-hidden rounded-[26px] border p-4 text-left transition duration-300 active:scale-[.985] ${selected ? 'border-white/35 bg-white/[.075] shadow-[0_18px_60px_rgba(0,0,0,.32)]' : 'border-white/[.09] bg-white/[.028] hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[.045]'}`}>
              ...
              <span className="relative flex h-full flex-col justify-between gap-7">
                <span className="flex items-start justify-between gap-3"><span className="grid h-11 w-11 place-items-center rounded-[15px] border border-white/15 bg-black/22 text-white shadow-[inset_0_1px_0_rgba(255,255,255,.14)]" aria-hidden><DomainIcon size={21} /></span><span className="text-3xl font-black tracking-[-.06em] text-white/90">{count}</span></span>
                <span className="block min-w-0"><span className="flex items-center gap-2"><span className={`h-1.5 w-1.5 rounded-full ${item.accent}`} aria-hidden /><span className="truncate text-lg font-black tracking-tight">{item.short}</span></span><span className="mt-1 block truncate text-[10px] font-bold text-white/48">{item.hint}</span></span>
              </span>
            </button>
          )
        })}
      ...
      <div className="relative mt-8 rounded-[26px] border border-white/[.075] bg-white/[.022] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,.035)] backdrop-blur-xl sm:p-4">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          {visible.map((feature) => {
            const featureCategory = categoryOf(feature)
            return (
              <Link key={`${feature.to}-${feature.nama}`} to={canonical(feature.to)} className={`group min-h-[82px] rounded-[18px] border p-3 transition duration-200 hover:-translate-y-px active:scale-[.985] ${featureSurface(featureCategory)}`} aria-label={`${feature.nama} · ${featureCategory}`}>
                <div className="flex items-start justify-between gap-2"><span className="truncate text-[8px] font-black uppercase tracking-[.12em] text-white/38">{featureCategory}</span><span className="text-[10px] text-white/26 transition group-hover:text-white/68" aria-hidden>↗</span></div>
