...
function OpticsSchematic({ pupilMm, distanceM }: { pupilMm: number; distanceM: number }) {
  const accommodationD = distanceM > 0 ? 1 / distanceM : 0
  const pupilHalf = 8 + ((pupilMm - 2) / 6) * 18
  const lensBulge = clamp(accommodationD / 4, 0, 1)
  const lensRx = 17 - lensBulge * 5
  ...
  return (
    <svg viewBox="0 0 760 270" className="h-auto w-full" role="img" aria-label="Educational ocular optics schematic">
      ...
      <text x="24" y="30" fill="#8ba2b5" fontSize="11" fontWeight="700">OPTICAL SCHEMATIC · EDUCATIONAL, NOT PATIENT BIOMETRY</text>
      ...
      <line x1="365" y1={140 - pupilHalf} x2="365" y2={140 - 4} stroke="#5e7890" strokeWidth="12" strokeLinecap="round" />
      <line x1="365" y1={140 + 4} x2="365" y2={140 + pupilHalf} stroke="#5e7890" strokeWidth="12" strokeLinecap="round" />
      ...
      <text x="293" y="246" fill="#a9bdcc" fontSize="10">cornea</text>
      <text x="346" y="246" fill="#a9bdcc" fontSize="10">pupil</text>
      <text x="392" y="246" fill="#a9bdcc" fontSize="10">lens</text>
...
export function Ocular4DAtlas() {
  ...
  const activeStructure = STRUCTURES.find((item) => item.term === selectedStructure) ?? visibleStructures[0] ?? STRUCTURES[0]
  const accommodationD = distanceM > 0 ? 1 / distanceM : 0
  const focalLengthM = accommodationD > 0 ? 1 / accommodationD : Infinity
      ...
      <section className="overflow-hidden rounded-[30px] border border-neutral-200 bg-white shadow-sm dark:border-white/10 dark:bg-[#080c10]">
        <div className="border-b border-neutral-200 p-4 dark:border-white/10 sm:p-5">
          ...
          <div className="no-scrollbar mt-4 flex gap-2 overflow-x-auto pb-1">
            {GROUPS.map((item) => (
              <button key={item.key} onClick={() => chooseGroup(item.key)} className={`min-w-[170px] shrink-0 rounded-2xl border p-3 text-left transition ${item.key === groupKey ? 'border-sky-300 bg-sky-50 dark:border-sky-300/30 dark:bg-sky-300/10' : 'border-neutral-200 bg-neutral-50 dark:border-white/10 dark:bg-white/[.025]'}`}>
      ...
      <section className="rounded-[30px] border border-neutral-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#080c10] sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-[10px] leading-relaxed text-neutral-500 dark:text-neutral-400">The timeline explains sequence and mechanism. It does not pretend that a static reference mesh is a live eye or patient-specific optical biometry.</p>
          </div>
        ...
        <div className="no-scrollbar mt-4 flex gap-2 overflow-x-auto pb-1">
          {VISION_PHASES.map((phase, index) => (
            <button key={phase.label} onClick={() => setPhaseIndex(index)} className={`min-w-[205px] shrink-0 rounded-2xl border p-3 text-left ${phaseIndex === index ? 'border-violet-300 bg-violet-50 dark:border-violet-300/30 dark:bg-violet-300/10' : 'border-neutral-200 bg-neutral-50 dark:border-white/10 dark:bg-white/[.025]'}`}>
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1.3fr)_minmax(300px,.7fr)]">
          ...
          <div className="space-y-3">
            ...
            <article className="rounded-2xl border border-neutral-200 bg-neutral-50 p-3 dark:border-white/10 dark:bg-white/[.025]">
              <label className="flex items-center justify-between text-[9px] font-black uppercase tracking-wide text-neutral-500"><span>Pupil aperture</span><span>{pupilMm.toFixed(1)} mm</span></label>
              <input type="range" min="2" max="8" step="0.5" value={pupilMm} onChange={(event) => setPupilMm(Number(event.target.value))} className="mt-2 w-full" />
              <label className="mt-4 flex items-center justify-between text-[9px] font-black uppercase tracking-wide text-neutral-500"><span>Target distance</span><span>{distanceM < 1 ? distanceM.toFixed(2) : distanceM.toFixed(1)} m</span></label>
              <input type="range" min="0.25" max="6" step="0.05" value={distanceM} onChange={(event) => setDistanceM(Number(event.target.value))} className="mt-2 w-full" />
            </article>
