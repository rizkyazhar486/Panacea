...
import { OcularGazeSimulator } from './OcularGazeSimulator'

type GazeKey = 'primary' | 'right' | 'left' | 'up' | 'down' | 'up-right' | 'up-left' | 'down-right' | 'down-left'
type Muscle = { label: string; term: string; ontology: string; nerve: string; principal: string; secondary: string }
...
const GAZE: Record<GazeKey, { label: string; active: string[]; explanation: string }> = {
  ...
  up: { label: 'Up', active: ['Superior rectus', 'Inferior oblique'], explanation: 'Elevation uses coordinated vertical actions; contribution depends on eye position.' },
  down: { label: 'Down', active: ['Inferior rectus', 'Superior oblique'], explanation: 'Depression uses coordinated vertical actions; contribution depends on eye position.' },
...
export function OcularMotility4D() {
  ...
  return (
    <section className="space-y-4 rounded-[30px] border border-neutral-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#080c10] sm:p-5">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between"><div><div className="text-[9px] font-black uppercase tracking-[.17em] text-fuchsia-700 dark:text-fuchsia-300">Ocular motility 4D</div><h3 className="mt-1 text-lg font-black text-neutral-950 dark:text-white">Extraocular muscles · gaze mechanics · eyelid elevator</h3><p className="mt-1 max-w-3xl text-[10px] leading-relaxed text-neutral-500 dark:text-neutral-400">Source-resolved anatomy is paired with a clearly schematic binocular motion simulator. Source geometry is shown only when available; the teaching animation never invents EMG, force or nerve-firing percentages.</p></div><span className="rounded-full border border-neutral-200 bg-neutral-50 px-3 py-1.5 text-[9px] font-black text-neutral-500 dark:border-white/10 dark:bg-white/[.03] dark:text-neutral-300">CN III · IV · VI teaching map</span></div>
      <OcularGazeSimulator />
      <div className="grid gap-3 lg:grid-cols-[270px_minmax(0,1fr)]"><div className="space-y-2">{MUSCLES.map((muscle) => <button key={muscle.term} onClick={() => setSelected(muscle.term)} className={`w-full rounded-2xl border p-3 text-left ${muscle.term === selectedMuscle.term ? 'border-fuchsia-300 bg-fuchsia-50 dark:border-fuchsia-300/30 dark:bg-fuchsia-300/10' : 'border-neutral-200 bg-neutral-50 dark:border-white/10 dark:bg-white/[.025]'}`}><div className="flex items-start justify-between gap-2"><span className="text-[10px] font-black text-neutral-950 dark:text-white">{muscle.label}</span><span className="text-[8px] font-black text-fuchsia-700 dark:text-fuchsia-300">{muscle.nerve}</span></div><div className="mt-1 text-[9px] font-bold text-neutral-600 dark:text-neutral-300">{muscle.principal}</div><div className="mt-1 text-[8px] leading-relaxed text-neutral-400">{muscle.ontology}</div></button>)}</div><HraResolvedAnatomyViewer key={selectedMuscle.term} title={`${selectedMuscle.label} · source-resolved HRA geometry`} description={`${selectedMuscle.principal}. ${selectedMuscle.secondary} Innervation label: ${selectedMuscle.nerve}. If the current source release contains only a mapping record, this panel deliberately shows no substitute 3D muscle.`} terms={[selectedMuscle.term, 'ocular muscle']} maxResults={14}/></div>
      <div className="rounded-[26px] border border-neutral-200 bg-neutral-50 p-3 dark:border-white/10 dark:bg-white/[.025] sm:p-4"><div className="grid gap-3 lg:grid-cols-[250px_minmax(0,1fr)] lg:items-center"><div className="grid grid-cols-3 gap-1.5">{GAZE_ORDER.map((key) => <button key={key} onClick={() => setGaze(key)} className={`min-h-14 rounded-xl border px-2 py-2 text-[9px] font-black ${gaze === key ? 'border-fuchsia-500 bg-fuchsia-500 text-white' : 'border-neutral-200 bg-white text-neutral-500 dark:border-white/10 dark:bg-white/[.035] dark:text-neutral-300'}`}>{GAZE[key].label}</button>)}</div><div><div className="text-[9px] font-black uppercase tracking-wide text-neutral-400">Muscle test map · {gazeState.label}</div><p className="mt-2 text-[11px] leading-relaxed text-neutral-700 dark:text-neutral-200">{gazeState.explanation}</p>{gazeState.active.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{gazeState.active.map((item) => <span key={item} className="rounded-full border border-fuchsia-200 bg-fuchsia-50 px-2.5 py-1 text-[8px] font-black text-fuchsia-800 dark:border-fuchsia-300/20 dark:bg-fuchsia-300/10 dark:text-fuchsia-200">{item}</span>)}</div>}</div></div></div>
