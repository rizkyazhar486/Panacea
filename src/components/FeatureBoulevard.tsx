...
type Zone = 'home' | 'learn' | 'body' | 'services' | 'clinical' | 'for-you' | 'all'

type Placement = { zone: 'home' | 'learn' | 'body' | 'services'; aisle: string }
...
function canonical(to: string): string {
  const learn: Record<string, string> = {
    ...
    '/genome-lab': '/genome-lab',
    '/knowledge-bridge': '/knowledge-bridge',
    '/med-study': '/med-study',
    '/osce-ukmppd': '/osce-ukmppd',
    '/evidence': '/rujukan?t=bukti',
    '/drug-info': '/rujukan?t=obat',
...
function accepts(zone: Zone, placement: Placement['zone']) {
  if (zone === 'all') return true
  if (zone === 'clinical') return placement === 'learn' || placement === 'services'
  if (zone === 'for-you') return placement === 'home'
...
export function FeatureBoulevard({ zone = 'all', title = 'Feature Boulevard' }: { zone?: Zone; title?: string }) {
  ...
  const placed = useMemo(() => {
    ...
    return FITUR_DARI_HUB.flatMap((f) => {
      const key = `${f.to}|${f.nama}`
      if (seen.has(key)) return []
      seen.add(key)
      const placement = place(f.to, f.grup, f.nama, `${f.apa} ${f.kw}`)
      if (!accepts(zone, placement.zone)) return []
      return [{ ...f, ...placement }]
    })
  }, [zone])
  ...
  return (
    <section className="relative isolate overflow-hidden rounded-[26px] border border-white/10 bg-[#020509]/95 p-3 text-white shadow-[0_24px_80px_rgba(0,0,0,.44)] backdrop-blur-2xl sm:rounded-[30px] sm:p-5" aria-label={title}>
      ...
      <label className="relative mt-3 flex min-h-[46px] items-center gap-2 rounded-[16px] border border-white/10 bg-black/55 px-3 shadow-[inset_0_1px_0_rgba(255,255,255,.06)] backdrop-blur-xl transition focus-within:border-cyan-300/35 focus-within:bg-cyan-300/[.035]">
        <span aria-hidden className="text-cyan-200/75">⌕</span>
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search capability…" className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-white outline-none placeholder:text-white/32" />
        {q && <button type="button" onClick={() => setQ('')} className="grid h-8 w-8 place-items-center rounded-[10px] border border-white/10 bg-white/[.045] text-cyan-200" aria-label="Clear search">×</button>}
      </label>
        ...
        {rows.map(([group, items]) => (
          <div key={group}>
            ...
            <div className="no-scrollbar flex snap-x snap-mandatory gap-2 overflow-x-auto overscroll-x-contain pb-1">
              {items.map((item) => (
                <Link key={`${item.to}-${item.nama}`} to={canonical(item.to)} className="group min-h-[84px] w-[178px] shrink-0 snap-start rounded-[18px] border border-white/[.08] bg-white/[.035] p-3 transition duration-200 hover:-translate-y-px hover:border-cyan-200/25 hover:bg-cyan-300/[.055] active:scale-[.985] sm:w-[204px]">
                  <div className="flex items-start justify-between gap-2"><span className="truncate text-[8px] font-black uppercase tracking-[.14em] text-cyan-100/45">{item.grup}</span><span className="text-white/30 transition group-hover:text-cyan-100/80" aria-hidden>↗</span></div>
                  <div className="mt-3 line-clamp-2 text-[11px] font-black leading-snug text-white/86">{item.nama}</div>
                </Link>
              ))}
            </div>
          </div>
        ))}
        {rows.length === 0 && <div className="grid min-h-[120px] place-items-center rounded-[20px] border border-dashed border-white/10 text-xs font-black text-white/40">No matching capability</div>}
      </div>
    </section>
  )
}
