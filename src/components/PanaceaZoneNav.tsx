...
export function PanaceaZoneNav() {
  ...
  return (
    <nav className="relative flex min-h-[58px] items-center gap-2 rounded-[20px] border border-white/[.085] bg-[#01040a]/88 px-2 py-1.5 shadow-[0_16px_46px_rgba(0,0,0,.25)] backdrop-blur-2xl sm:min-h-[64px] sm:gap-3 sm:rounded-[22px] sm:px-3" aria-label="Panacea super pages">
      <NavLink to="/" className="flex h-11 shrink-0 items-center gap-2 rounded-[15px] px-1.5 text-white transition hover:bg-white/[.04] sm:px-2" aria-label="Panacea Home">
        <LogoMark size={30} />
      ...
      <div className="no-scrollbar flex min-w-0 flex-1 items-center gap-1.5 overflow-x-auto sm:justify-center sm:gap-2">
        {ZONES.map((zone) => (
          <NavLink key={zone.to} to={zone.to} className={({ isActive }) => `grid h-11 min-w-[82px] shrink-0 place-items-center rounded-[14px] border px-3 text-[10px] font-black transition duration-200 active:scale-[.98] sm:min-w-[104px] sm:px-4 sm:text-xs ${isActive ? 'border-cyan-100/45 bg-gradient-to-r from-cyan-200 via-emerald-200 to-violet-200 text-[#01040a] shadow-[0_8px_26px_rgba(34,211,238,.12)]' : 'border-transparent bg-transparent text-white/60 hover:border-white/[.09] hover:bg-white/[.045] hover:text-white'}`}>
            {zone.label}
          </NavLink>
        ))}
      </div>
