...
import { evaluasiAturanLab, LABEL_KEADAAN, validasiBarisAturanLab } from '../lib/aturanLabDokter'
...
export function RencanaHarianDokter({ izinId, state }: { izinId: string; state?: LongitudinalPatientState }) {
  ...
  // Baris tidak valid ditampilkan di bawah baris itu sendiri sebelum submit,
  // dengan pesan sama persis dengan yang akan ditolak server — dokter tidak
  // perlu menebak baris mana yang salah dari satu pesan galat generik.
  const aturanLabGalat = aturanLab.map(validasiBarisAturanLab)

  const simpan = () => {
    if (submitting) return
    if (aturanLabGalat.some((e) => e !== null)) { setGalat('Fix the lab rule error(s) marked below before starting.'); return }
    setSubmitting(true)
    ...
    }).then(() => { setGalat(null); void muat() }).catch((e) => setGalat((e as Error).message)).finally(() => setSubmitting(false))
  }
      ...
      {!plan ? (
        <div className="mt-1.5 grid gap-1.5">
            ...
            {aturanLab.map((a, i) => {
              ...
              return (
                <div key={i} className="grid gap-1 text-[11px] text-white/70">
                  ...
                  {aturanLabGalat[i] && <p role="alert" className="text-[10px] font-bold text-amber-300">Rule {i + 1}: {aturanLabGalat[i]}</p>}
                </div>
              )
            })}
          ...
          <div className="flex gap-1.5">
            ...
            <button type="button" disabled={submitting} className="ml-auto min-h-9 rounded-full px-3 text-[11px] font-black disabled:opacity-50" onClick={() => void simpan()}>{submitting ? 'Starting…' : 'Start daily check-in'}</button>
          </div>
        </div>
      ) : (
        <div className="mt-1">
