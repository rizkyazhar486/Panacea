...
// Mirrors server/src/carePlan.ts's teks()/threshold checks for lab review rules (the frontend
// build root excludes server/, so this is kept in sync by hand) — surfaces the same wording
// before the round trip instead of only after a rejected save.
...
function validasiAturanLab(a: AturanLab): string | null {
  ...
  if (!a.bukti.trim() || a.bukti.trim().length > 300) return 'evidence reference is required (max 300 characters)'
  return null
}
...
export function RencanaHarianDokter({ izinId, state }: { izinId: string; state?: LongitudinalPatientState }) {
  ...
  const aturanLabGalat = aturanLab.map(validasiAturanLab)
      ...
      {!plan ? (
        <div className="mt-1.5 grid gap-1.5">
            ...
            {aturanLab.map((a, i) => {
              ...
              return (
                <div key={i} className="grid gap-1 text-[11px] text-white/70">
                  ...
                  {aturanLabGalat[i] && <p role="alert" className="text-[10px] font-bold text-amber-300">{aturanLabGalat[i]}</p>}
                </div>
              )
            })}
          ...
          <div className="flex gap-1.5">
            ...
            <button type="button" disabled={submitting || aturanLabGalat.some(Boolean)} aria-busy={submitting}
              className="ml-auto min-h-9 rounded-full px-3 text-[11px] font-black disabled:opacity-50" onClick={() => void simpan()}>
              {submitting ? 'Saving…' : 'Start daily check-in'}
            </button>
          </div>
        </div>
      ) : (
