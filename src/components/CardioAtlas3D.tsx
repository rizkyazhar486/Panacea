...
// ─────────────────────────────────────────────────────────────────────────────
// Penampil kardiovaskular: 131 struktur bernama, aliran yang bergerak, dan
// lesi yang ditunjukkan DI TEMPATNYA.
//
// Tiga hal yang membuatnya berbeda dari penampil organ biasa:
//
//   1. TIAP MESH BERNAMA. Menyorot "LAD" cukup dengan mencocokkan nama, tanpa
//      daftar indeks yang akan basi begitu berkasnya dibangun ulang.
//   2. ALIRAN MENGIKUTI GARIS TENGAH PEMBULUH, bukan garis lurus antar organ.
...
//      pembuluh lain menghapus konteks letaknya; meredupkan tetap menjawab
//      "di sebelah mana ini".
// ─────────────────────────────────────────────────────────────────────────────
...
export interface CardioProps {
  ...
  hr?: number
  /** Hanya tampilkan wilayah ini (heart, coronary, head, …). null = semua. */
  wilayah?: string | null
...
export function CardioAtlas3D({ lesi = [], hilir = [], jalur = null, hr = 72, wilayah = null, onPilih, dipilih = null }: CardioProps) {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        grup = gltf.scene
        const asliPerBersih = new Map(CARDIO_PARTS.map((p) => [namaBersih(p.name), p]))
        grup.traverse((o) => {
          ...
          wilayahMesh.set(m, bagian?.region ?? '')
        ...
        const takDikenal = meshes.filter((m) => !wilayahMesh.get(m)).length
        if (takDikenal) console.warn(`${takDikenal} mesh kardiovaskular tidak dikenali namanya`)
        scene.add(grup)
        setMuat(false)
      },
      (ev) => { if (ev.total > 0) setPct(ev.loaded / ev.total) },
      () => { setGagal('Could not load the cardiovascular model.'); setMuat(false) },
    )
    ...
    function bingkai() {
      ...
      if (meshes.length && kunciBaru !== kunciBingkai) {
        ...
        if (adaSorot) bingkaiKe((m) => setLesi.has(m.name.toLowerCase()) || setHilir.has(m.name.toLowerCase()), 3.2)
        else if (W || J) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-neutral-50 dark:bg-white/5">
      <div ref={wadahRef} className="h-[300px] w-full" />
      {muat && !gagal && (
        <div className="absolute inset-0 flex items-center justify-center">
          <p className="text-xs font-semibold text-neutral-500">
            Loading vessels… {pct > 0 ? `${Math.round(pct * 100)}%` : ''}
          </p>
        </div>
      )}
      {gagal && (
      ...
      {!muat && !gagal && (
        <p className="px-3 pb-2 text-center text-[10.5px] text-neutral-400">
          {sentuh ? sentuh : 'Drag to rotate · tap any vessel to see what goes wrong there'}
        </p>
      )}
    </div>
  )
