...
export function ActivityShareCard({ data, onClose }: { data: ActivityShareData; onClose: () => void }) {
  ...
  async function handleDownload() { setBusy(true); const blob = await toBlob(); if (blob) download(blob); setBusy(false) }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="max-h-[92vh] w-full max-w-sm overflow-y-auto rounded-3xl bg-white p-4 dark:bg-neutral-950">
        ...
        <button type="button" onClick={() => setIncludeRoute((x) => !x)}
          ...
          <p className="mt-1 text-[10px] leading-relaxed text-neutral-500">When enabled, approximately the first and last 200 m are removed. If the route is too short to protect both ends, the card keeps the route hidden.</p>
        </button>
        {includeRoute && privacyPoints.length === 0 && <p className="mt-2 text-[10px] font-semibold text-amber-700 dark:text-amber-300">This route is too short for the 200 m privacy buffer, so it remains hidden.</p>}
