...
export default function BodyAllSystems3D() {
  ...
  const systemRef = useRef<BodySystemId>(systemId)
  ...
  useEffect(() => {
    ...
    const sourceMaterials = new Map<THREE.Material, { systemIds: Set<BodySystemId>; targetIds: Set<string> }>()
    const sourceBounds = new THREE.Box3()
    let disposed = false

    const resolvedByFile = new Map<string, Map<string, { systems: Set<BodySystemId>; targets: Set<string> }>>()
    for (const system of resolveBodySystemSourceWave()) {
      for (const target of system.targets) {
        const fileMap = resolvedByFile.get(target.file) ?? new Map()
        for (const name of target.names) {
          const keyName = normalizeAnatomySourceName(name)
          const meta = fileMap.get(keyName) ?? { systems: new Set<BodySystemId>(), targets: new Set<string>() }
          meta.systems.add(system.id)
          meta.targets.add(target.id)
          fileMap.set(keyName, meta)
        }
        resolvedByFile.set(target.file, fileMap)
      }
    }
    ...
    const updateVisibility = () => {
      ...
      for (const [file, root] of roots) {
        const fileMap = resolvedByFile.get(file)
        root.traverse((object) => {
          if (!(object as THREE.Mesh).isMesh) return
          const mesh = object as THREE.Mesh
          const meta = fileMap?.get(normalizeAnatomySourceName(mesh.name))
          mesh.visible = Boolean(meta?.systems.has(active))
          const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material]
          for (const material of materials) {
            const record = sourceMaterials.get(material)
            if ((material as THREE.MeshStandardMaterial).isMeshStandardMaterial) {
              const standard = material as THREE.MeshStandardMaterial
              standard.opacity = record?.systemIds.has(active) ? 0.9 : 0.12
              standard.emissive.set(record?.systemIds.has(active) ? 0x083c35 : 0x000000)
              standard.emissiveIntensity = record?.systemIds.has(active) ? 0.34 : 0
            }
          }
        })
      }
    ...
    let completed = 0
    const filesNeeded = FILE_ORDER.filter((file) => resolvedByFile.has(file))

    for (const file of filesNeeded) {
      loader.load(`${import.meta.env.BASE_URL}anatomy/${file}`, (gltf) => {
        if (disposed) return
        const root = gltf.scene
        const fileMap = resolvedByFile.get(file) ?? new Map()
        root.traverse((object) => {
          if (!(object as THREE.Mesh).isMesh) return
          const mesh = object as THREE.Mesh
          const meta = fileMap.get(normalizeAnatomySourceName(mesh.name))
          mesh.visible = Boolean(meta?.systems.has(systemRef.current))
          if (!meta) return
          ...
          for (const material of cloned) {
            sourceMaterials.set(material, { systemIds: new Set(meta.systems), targetIds: new Set(meta.targets) })
          }
    ...
    <section className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-3 dark:border-emerald-300/20 dark:bg-emerald-300/[.04]" aria-label="All body systems source-backed 3D">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-[10px] leading-relaxed text-neutral-600 dark:text-neutral-300">Every system resolves against the shipped GLB source index first. Unresolved structures stay unavailable; no replacement geometry, inferred anatomy, patient geometry or academic-review claim is created.</p>
        </div>
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-[1.1fr_.9fr]">
        <div className="relative min-h-[360px] overflow-hidden rounded-2xl border border-neutral-200 bg-neutral-950 dark:border-white/10">
          {open ? <div ref={containerRef} className="h-[360px] w-full sm:h-[430px]" aria-hidden="true" /> : <div className="flex h-[360px] items-center justify-center px-6 text-center text-xs font-bold text-neutral-500">Open the renderer to lazy-load only shipped anatomy GLBs.</div>}
