    ...
    <div className={`rounded-xl border ${warna}`}>
      ...
      >
        ...
        <svg
          ...
          className={`shrink-0 text-neutral-400 transition-transform duration-300 ${buka ? 'rotate-180' : ''}`}
        >
  ...
  return (
    <div className="rounded-xl bg-white/60 p-2.5 text-center">
      ...
      <div className="text-lg font-black leading-none tabular-nums" style={{ color: warna }}>
        {nilai}
        {satuan && <span className="ml-0.5 text-[10px] font-bold text-neutral-400">{satuan}</span>}
      </div>
      <div className="mt-1 text-[9px] font-bold uppercase leading-tight tracking-wide text-neutral-400">
        {label}
      </div>
    </div>
  )
