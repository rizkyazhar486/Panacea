...
export const GROUPS: { title: string; emoji: string; tools: Tool[] }[] = [
  {
    ...
    tools: [
      ...
      { to: '/connect', name: 'Connect', what: 'Pair and manage connected devices and services', kw: 'connect device perangkat sambungkan wearable integrasi', tag: 'Core' },
      { to: '/analisis-gerak', name: 'Analisis Gerak', what: 'Walking asymmetry, gait quality, running form and heart-rate recovery, read from your watch export', kw: 'gait berjalan asimetri asymmetry langkah step length double support cadence irama ground contact vertical oscillation running form bentuk lari hrr cardio recovery pemulihan stair speed tangga daylight cahaya headphone audio', tag: 'Core' },
      { to: '/riwayat-latihan', name: 'Riwayat Latihan', what: 'Every imported session with its per-minute heart-rate curve, zone split and recovery', kw: 'workout latihan riwayat history sesi heart rate curve kurva zona zone 80/20 easy pace hrr recovery pemulihan apple watch import notification peringatan denyut', tag: 'Core' },
      { to: '/log-detak-jantung', name: 'Log Detak Jantung', what: 'Every heart-rate sample your watch pushes, with how dense the data really is', kw: 'heart rate log detak jantung real time realtime live monitor sampel webhook auto export apple watch bpm', tag: 'Core' },
      { to: '/pola-tidur', name: 'Pola Tidur', what: 'Sleep by stage per night — deep, REM, core, awake — plus bedtime regularity', kw: 'tidur sleep pola stage deep rem core awake tahapan malam jam tidur keteraturan regularity apple watch jaga shift', tag: 'Core' },
