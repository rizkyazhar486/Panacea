...
export function NutritionChecklist() {
  ...
  return (
    <section
      ...
      aria-labelledby="nutrition-checklist-title"
      aria-describedby="nutrition-checklist-description"
    >
      <div className="rounded-3xl border border-emerald-100 bg-white/90 p-4 shadow-sm dark:border-emerald-400/15 dark:bg-white/[0.04] sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="max-w-2xl">
            ...
            <p id="nutrition-checklist-description" className="mt-1 text-sm leading-relaxed text-neutral-600 dark:text-neutral-300">
          ...
          <div
            className="rounded-2xl bg-emerald-50 px-3 py-2 text-right dark:bg-emerald-400/10"
            role="status"
            aria-live="polite"
            aria-label={`${reviewed} of ${ITEMS.length} nutrition checklist items reviewed`}
          >
        ...
        <ul className="mt-4 space-y-2" aria-label="Nutrition data safety checklist">
          {ITEMS.map((item) => {
            const detailId = `nutrition-check-${item.id}-detail`
            return (
              <li key={item.id} data-check-id={item.id} className="rounded-2xl border border-neutral-100 p-3 dark:border-white/10">
                <label className="flex cursor-pointer items-start gap-3">
                  <input
                    ...
                    className="mt-1 h-4 w-4 shrink-0 accent-emerald-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2"
                  />
                  <span>
                    <span className="block text-sm font-black text-ink dark:text-white">{item.label}</span>
                    <span id={detailId} className="mt-0.5 block text-xs leading-relaxed text-neutral-600 dark:text-neutral-400">{item.detail}</span>
                  </span>
                </label>
              </li>
            )
