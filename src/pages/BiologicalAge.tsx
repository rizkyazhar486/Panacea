    ...
    <div className="mx-auto max-w-2xl space-y-5 pb-24">
      <Card className="!p-5">
        <SectionTitle
          ...
          subtitle="A directional number, not a validated biological age."
          ...
          <div className="rounded-2xl border border-neutral-100 p-4 text-center dark:border-white/10">
            ...
            {bioDelta != null && (
              <Badge tone={bioDelta <= -1 ? 'brand' : bioDelta <= 1 ? 'low' : 'critical'}>
                {bioDelta <= 0 ? `${Math.abs(bioDelta)} years younger` : `${bioDelta} years older`}
              </Badge>
            )}
        ...
        <p className="mt-3 rounded-xl bg-amber-500/10 px-3 py-2 text-[11px] leading-snug text-amber-800 dark:text-amber-300">
          The weight given to each marker here is <b>chosen by the author</b>, not drawn from research. Use its direction
          and how it changes over time — not the number itself as a true age. Validated biological clocks (e.g. epigenetic) measure something else.
        </p>
      </Card>

      {/* Inputs */}
      <Card className="!p-5">
        <SectionTitle icon={<IconActivity size={20} />} title="Your Numbers" subtitle="Most fields fill in automatically from Health Profile & Body Composition — add lab results if you have them" />
      ...
      {markers.length > 0 && (
        <Card className="!p-5">
          <SectionTitle icon={<IconChartUp size={20} />} title="What is moving your age" subtitle="Each marker shifts your biological age up or down (in years)" />
