  ...
  return (
    <div className={`rounded-xl p-2.5 ${kosong ? 'border border-dashed border-neutral-300 dark:border-white/15' : 'bg-black/[0.03] dark:bg-white/5'}`}>
      ...
      <span className="flex items-baseline gap-1">
        <span className={`text-[17px] font-black leading-none tabular-nums ${kosong ? 'text-neutral-300 dark:text-neutral-600' : 'text-ink dark:text-white'}`}>{p.nilai}</span>
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 px-fluid pb-24">
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => setGeser((g) => g - 1)} aria-label="Previous day" className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-neutral-100 text-lg font-black dark:bg-white/10">‹</button>
        <div className="min-w-0 flex-1 text-center">
          <span className="block truncate text-[14px] font-black text-ink dark:text-white">{judulTanggal(tanggalObj)}</span>
          <span className="block text-[11px] text-neutral-500">{geser === 0 ? 'Today' : geser === -1 ? 'Yesterday' : `${Math.abs(geser)} days ago`}</span>
        </div>
        <button onClick={() => setGeser((g) => Math.min(0, g + 1))} disabled={geser === 0} aria-label="Next day" className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-neutral-100 text-lg font-black disabled:opacity-30 dark:bg-white/10">›</button>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {pekan.map((h) => {
          ...
          const depan = h.g > 0
          return (
            <button key={h.kunci} disabled={depan} onClick={() => setGeser(h.g)} className={`flex min-h-[52px] flex-col items-center justify-center rounded-xl text-[10px] font-bold transition disabled:opacity-25 ${kini ? 'bg-brand text-white' : 'bg-neutral-100 text-neutral-600 dark:bg-white/10 dark:text-neutral-300'}`}>
      ...
      <Card className="!p-3">
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">{terukur.map((p) => <Petak key={p.label} p={p} />)}</div>
        {sesiHari.length > 0 && <div className="mt-2 space-y-1">{sesiHari.map((s) => (
          <div key={s.id} className="flex items-baseline justify-between gap-2 rounded-xl bg-black/[0.03] px-2.5 py-2 dark:bg-white/5">
            ...
            <span className="shrink-0 text-[11px] tabular-nums text-neutral-500">{s.jarakKm ? `${s.jarakKm.toFixed(1)} km · ` : ''}{fmtDurasi(s.durasi)}{s.jarakKm && s.durasi ? ` · ${fmtPace(Math.round(s.durasi / s.jarakKm))}/km` : ''}{s.avgHr ? ` · ${Math.round(s.avgHr)} bpm` : ''}</span>
          </div>
        ))}</div>}
        {sesiHari.length === 0 && totalDetik === 0 && <p className="mt-2 text-[11px] text-neutral-400">No sessions stored for this day.</p>}
      </Card>
      ...
      <Card className="!p-3">
        ...
        <div className="mt-2">
          ...
          <div className="mt-1 flex gap-1">{['Empty', 'Low', 'Normal', 'Fresh', 'Full'].map((l, i) => (
            <button key={l} onClick={() => { const n = i + 1; setTenaga(n); logWellness(tgl, { tenaga: n }) }} aria-pressed={tenaga === i + 1} className={`flex min-h-[40px] flex-1 items-center justify-center rounded-xl text-[11px] font-bold transition ${tenaga === i + 1 ? 'bg-brand text-white' : 'bg-neutral-100 text-neutral-600 dark:bg-white/10 dark:text-neutral-300'}`}>{l}</button>
          ))}</div>
        </div>
        <div className="mt-3">
          <label className="text-[11px] font-bold text-neutral-500" htmlFor="catatan-harian">Notes</label>
          <textarea id="catatan-harian" value={catatan} maxLength={280} onChange={(e) => setCatatan(e.target.value)} onBlur={() => logWellness(tgl, { catatan: catatan.trim() })} placeholder="How it felt, where it hurt, the weather…" className="mt-1 h-20 w-full rounded-xl border border-neutral-200 bg-white p-2.5 text-[13px] text-ink placeholder:text-neutral-400 dark:border-white/15 dark:bg-white/10 dark:text-white" />
          <span className="text-[10px] text-neutral-400">{catatan.length}/280 · saved when you leave the box</span>
        </div>
        <p className="mt-2 text-[11px] leading-snug text-neutral-500">Energy and notes are <b>never fed into the training-load model</b>. A self-reported value cannot be equated with a measured one — and it is precisely when the two disagree that the disagreement tells you something.</p>
      </Card>
    </div>
  )
}
