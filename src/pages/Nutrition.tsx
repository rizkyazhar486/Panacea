...
export function Nutrition() {
  ...
  const sleepHr = (w as Record<string, number | undefined>).sleepHr ?? 0
  const waterMl = (w as Record<string, number | undefined>).waterMl ?? 0
  const exerciseKcal = (w as Record<string, number | undefined>).exerciseKcal ?? 0
...
function ActivityLog() {
  ...
  const w = (state.wellness?.[today()] ?? {}) as Record<string, number | undefined>
        ...
        <div className="rounded-2xl border border-neutral-100 p-4 transition hover:shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-xl bg-orange-50 px-3 py-1.5 text-sm font-extrabold text-orange-600 tabular-nums">
              <AnimatedNum value={w.exerciseKcal ?? 0} /> kkal
            </span>
          </div>
          <div className="mt-3 flex flex-wrap items-end gap-3">
...
function WellnessTrendChart() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const wd = state.wellness?.[date] as Record<string, number | undefined> | undefined
            ...
            <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ minWidth: 320 }}>
              <defs>
                <linearGradient id="gK" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={COL_KCAL} stopOpacity="0.9" />
                  <stop offset="100%" stopColor={COL_KCAL} stopOpacity="0.5" />
                </linearGradient>
                <linearGradient id="gS" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={COL_SLEEP} stopOpacity="0.75" />
                  <stop offset="100%" stopColor={COL_SLEEP} stopOpacity="0.4" />
                </linearGradient>
                <linearGradient id="gW" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={COL_WATER} stopOpacity="0.8" />
                  <stop offset="100%" stopColor={COL_WATER} stopOpacity="0.45" />
                </linearGradient>
              </defs>
              {[0, 1, 2, 3].map((u) => (
                <line
...
function ChronicMonitor() {
  ...
  const daysLeft = state.chronicSubExpires
    ? Math.max(0, Math.ceil((new Date(state.chronicSubExpires).getTime() - Date.now()) / 86400000))
    : 0
  ...
  async function buy(plan: 'monthly' | 'lifetime') {
    ...
    try {
        ...
        const iv = setInterval(async () => {
          ...
          try {
            ...
            if (s.status === 'paid') {
              clearInterval(iv)
              buyChronicSub(plan)
              setPayMsg('✅ Pembayaran berhasil!')
            } else if (s.status === 'failed') {
              clearInterval(iv)
              setPayMsg('Pembayaran gagal.')
            }
          } catch { /* retry */ }
      ...
      setPayMsg('Gagal memproses. Coba lagi.')
    }
  }
    ...
    return (
      <Card className="!p-5 border-2 border-brand/15">
        <SectionTitle
          ...
          title="Pemantauan Kronis"
          subtitle="TTV harian pasien kronis"
          right={<Badge tone="high">Langganan</Badge>}
        />
        <div className="mt-4 rounded-2xl border-2 border-dashed border-brand/25 bg-gradient-to-br from-brand/[0.03] to-transparent p-6">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <button
              onClick={() => buy('monthly')}
              className="group rounded-2xl border-2 border-neutral-200 p-4 text-left transition-all hover:border-brand/40 hover:shadow-md"
            >
              <div className="text-[10px] font-bold uppercase tracking-widest text-neutral-400">Bulanan</div>
              <div className="mt-1 text-2xl font-extrabold text-neutral-800">
                Rp{CHRONIC_MONTHLY.toLocaleString('id-ID')}
                <span className="text-sm font-medium text-neutral-400">/bln</span>
              </div>
              <div className="mt-1.5 text-[11px] text-neutral-500">Pemantauan penuh, perpanjang tiap bulan.</div>
            </button>
            <button
              onClick={() => buy('lifetime')}
              className="relative rounded-2xl border-2 border-brand bg-brand/[0.04] p-4 text-left transition-all hover:shadow-md"
            >
              <span className="absolute -top-2.5 right-3 rounded-full bg-brand px-2.5 py-0.5 text-[9px] font-bold text-white shadow-sm">
                Terbaik
              </span>
              ...
              <div className="mt-1 text-2xl font-extrabold text-brand-dark">
                Rp{(CHRONIC_LIFETIME / 1e6).toLocaleString('id-ID')}jt
              </div>
    ...
    <Card className="!p-5 border-2 border-brand/15">
          ...
          <div className="flex gap-2">
            <Link to="/consult">
              <Button variant="outline" className="h-8 text-xs rounded-xl">
                <IconStethoscope size={14} /> Konsultasi
              </Button>
            </Link>
            <Link to="/hospitals">
              <Button className="h-8 text-xs rounded-xl">
                <IconHospital size={14} /> SOS
              </Button>
            </Link>
          </div>
        </div>
      ...
      {status === 'warn' && (
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
          ...
          <Link to="/consult">
            <Button variant="outline" className="h-8 text-xs rounded-xl">Konsultasi</Button>
          </Link>
        </div>
      )}
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
        <span className="text-sm text-neutral-500">
          Status:{' '}
          <b style={{ color: STATUS_COLOR[status] }}>
            {latest ? STATUS_LABEL[status] : 'Belum ada data'}
          </b>
          {p.chronicConditions.length > 0 && (
            <span className="ml-1.5 text-[11px] text-neutral-400">· {p.chronicConditions.join(', ')}</span>
          )}
        </span>
        <Button
          variant="outline"
        ...
        >
          <IconPlus size={14} /> Catat TTV
        </Button>
      </div>

      {show && (
        <div className="mt-3 grid grid-cols-2 gap-2.5 rounded-2xl bg-neutral-50 p-4 sm:grid-cols-4">
          {([['sys', 'Sistolik'], ['dia', 'Diastolik'], ['hr', 'Nadi'], ['rr', 'RR'], ['temp', 'Suhu'], ['spo2', 'SpO₂'], ['glu', 'Gula (ops.)']] as const).map(
            ([k, label]) => (
          ...
          <div className="flex items-end">
            <Button onClick={save} className="h-[38px] rounded-xl">Simpan</Button>
          </div>
        </div>
      )}
      ...
      {latest ? (
        <>
            ...
            {evals.map((e) => (
              ...
              >
                ...
                <div className="pl-2">
                  ...
                  >
                    {e.value}{' '}
                    <span className="text-[11px] font-medium text-neutral-400">{e.unit}</span>
                  </div>
          ...
          <p className="mt-3 text-[10px] text-neutral-400">
            {vitals.length} catatan · terakhir {new Date(latest.takenAt).toLocaleString('id-ID')}
          </p>
        </>
      ) : (
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      <div className="rounded-2xl border border-neutral-100 p-3">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-lg bg-red-50 px-2 py-0.5 text-[10px] font-bold text-red-500 tabular-nums">
            {recent[recent.length - 1].systolic} mmHg
          </span>
        </div>
        <svg viewBox="0 0 280 44" className="mt-2 w-full">
          {makeLine(recent.map((v) => v.systolic), '#FF3131')}
        </svg>
      </div>
      {glu.length >= 2 && (
        <div className="rounded-2xl border border-neutral-100 p-3">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-lg bg-amber-50 px-2 py-0.5 text-[10px] font-bold text-amber-600 tabular-nums">
              {glu[glu.length - 1]} mg/dL
            </span>
          </div>
          <svg viewBox="0 0 280 44" className="mt-2 w-full">
            {makeLine(glu, '#f59e0b')}
...
function computeLongevity(i: {
  ...
  const exercise = clamp(
    (i.exerciseFreq >= 1 ? 60 : i.exerciseFreq * 60) + Math.min(40, (i.exerciseMin / 30) * 40),
  )
  const hydration = clamp(
    i.hydrationL >= 2 && i.hydrationL <= 3.5
      ? 100
      : 100 - Math.abs(i.hydrationL - 2.5) * 28,
  )
  const sleep = clamp(
    i.sleepHr >= 7 && i.sleepHr <= 9 ? 100 : 100 - Math.abs(i.sleepHr - 8) * 16,
  )
  const sun = clamp(
    !i.sunDone
      ? 40
      : i.sunHr >= 0.17 && i.sunHr <= 0.6
        ? 100
        : 100 - Math.abs(i.sunHr - 0.33) * 80,
  )
...
function LongevityCalculator() {
  ...
  function compute() {
    setResult(
      computeLongevity({ mainMeals, snacks, exerciseFreq, exerciseMin, hydrationL, sleepHr, sunDone, sunHr }),
    )
  }
  ...
  return (
    <Card className="!p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {subActive ? (
          <Badge tone="brand">{daysLeft} hari</Badge>
        ) : (
          <Badge tone="high">Langganan</Badge>
        )}
      </div>
          ...
          <InputBlock title="☀️ Sinar Matahari">
            <label className="flex items-center gap-2 py-1 text-sm">
              <input
                type="checkbox"
                checked={sunDone}
                onChange={(e) => setSunDone(e.target.checked)}
                className="h-4 w-4 accent-[#00BF63] rounded"
              />
              Sudah berjemur?
            </label>
            {sunDone && (
              <Stepper label="Durasi" value={sunHr} min={0} max={3} step={0.25} unit="jam" onChange={setSunHr} />
            )}
          </InputBlock>
          {subActive && (
            <Button onClick={compute} className="w-full">
              <IconSparkle size={16} /> Hitung Nilai Longevity
            </Button>
          )}
        </div>
          ...
          {!subActive ? (
            <div className="flex h-full flex-col items-center justify-center rounded-2xl border-2 border-dashed border-brand/30 bg-brand/[0.03] p-8 text-center">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand/10">
                <IconSparkle size={24} className="text-brand" />
              </div>
              ...
              <p className="mt-1 max-w-[240px] text-sm text-neutral-500">
                Langganan 30 hari <b>Rp{PRICE_LONGEVITY.toLocaleString('id-ID')}</b>
              </p>
              <Button className="mt-5" onClick={() => buyLongevitySub()}>
                <IconSparkle size={15} /> Langgani Sekarang
              </Button>
              <Link to="/marketplace" className="mt-2 text-xs font-semibold text-brand-dark hover:underline">
                Buka di Marketplace →
              </Link>
            </div>
          ) : result ? (
            <div className="rounded-2xl bg-gradient-to-br from-brand to-brand-dark p-5 text-white shadow-lg">
              ...
              <button
                onClick={shareToFeed}
                className="mt-4 w-full rounded-xl bg-white py-2.5 text-sm font-bold text-brand-dark transition hover:bg-white/90"
              >
  ...
  return (
    <div className="flex flex-col items-center">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        />
        <text x={size / 2} y={size / 2 - 8} textAnchor="middle" fontSize="30" fontWeight="800" fill="#111">
          {value}
        </text>
        <text x={size / 2} y={size / 2 + 14} textAnchor="middle" fontSize="10" fontWeight="600" fill="#aaa">
          / {max} {unit}
        </text>
      </svg>
...
function MacroCard({ label, value, target, color, emoji }: {
  ...
  return (
    <div className="bg-white px-4 py-3.5 text-center transition hover:bg-neutral-50">
      ...
      <div className="mt-0.5 text-[10px] font-bold uppercase tracking-widest text-neutral-400">{label}</div>
      <div className="mt-1 text-xl font-extrabold tabular-nums" style={{ color }}>
        {value}g
      </div>
...
function AnimatedNum({ value, decimals = 0 }: { value: number; decimals?: number }) {
  ...
  useEffect(() => {
    ...
    function tick(now: number) {
      ...
      const cur = start + (value - start) * ease
  ...
  return (
    <div className="flex items-center justify-between py-1">
      ...
      <div className="flex items-center gap-1.5">
        <button
          onClick={dec}
          className="grid h-7 w-7 place-items-center rounded-lg bg-neutral-100 text-sm font-bold text-neutral-600 transition hover:bg-neutral-200 active:scale-90"
        >
          −
        </button>
        ...
        <button
          onClick={inc}
          className="grid h-7 w-7 place-items-center rounded-lg bg-brand/10 text-sm font-bold text-brand-dark transition hover:bg-brand/20 active:scale-90"
        >
          +
        </button>
      </div>
    ...
    <div>
      ...
      <div className="h-1.5 overflow-hidden rounded-full bg-white/20">
        <div
          ...
          style={{ width: `${shown ? v : 0}%`, transition: 'width 0.8s cubic-bezier(0.22,0.61,0.36,1)' }}
        />
      </div>
    </div>
  )
...
function Gauge({ value }: { value: number }) {
  ...
  return (
    <svg width="84" height="84" viewBox="0 0 76 76" className="drop-shadow-lg">
      ...
      />
      <text x="38" y="42" textAnchor="middle" fontSize="18" fontWeight="800" fill="#fff">
        {value}
      </text>
    </svg>
  )
