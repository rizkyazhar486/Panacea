    ...
    <main className="mx-auto w-full max-w-4xl space-y-6 pb-24">
      <section className="overflow-hidden rounded-[28px] border border-emerald-200/60 bg-gradient-to-br from-emerald-50 via-white to-sky-50 p-4 shadow-[0_18px_45px_rgba(15,80,60,.08)] dark:border-white/10 dark:from-emerald-500/12 dark:via-[#0b1518] dark:to-sky-500/10 sm:p-5">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[10px] font-semibold uppercase tracking-[.16em] text-emerald-700/70 dark:text-emerald-300/70">My Panacea</div>
            <h1 className="mt-1 truncate text-[26px] font-semibold tracking-[-.035em] text-neutral-950 dark:text-white sm:text-[32px]">
              Hi{name ? `, ${name}` : ''}
            </h1>
          </div>
          <div className="flex shrink-0 gap-2">
            ...
            <Link
              to="/atur-fitur"
              className="grid h-11 w-11 place-items-center rounded-full bg-neutral-950 text-lg font-medium text-white shadow-sm transition active:scale-95 dark:bg-white dark:text-neutral-950"
        ...
        {signals.length > 0 ? (
          <div className="no-scrollbar -mx-1 mt-4 flex snap-x gap-2.5 overflow-x-auto px-1 pb-1">
            {signals.map((s) => (
              ...
              >
                <div className="text-[9px] font-semibold uppercase tracking-[.13em] text-neutral-500 dark:text-neutral-400">{s.label}</div>
                <div className={`mt-4 text-[30px] font-semibold leading-none tracking-[-.035em] tabular-nums ${s.tone}`}>{s.value}</div>
                {s.unit && <div className="mt-2 text-[10px] font-medium text-neutral-500 dark:text-neutral-400">{s.unit}</div>}
              </Link>
            ))}
          </div>
        ) : (
          <Link to="/harian" className="mt-4 flex min-h-[58px] items-center justify-between rounded-2xl bg-white/90 px-4 text-sm font-medium text-neutral-800 shadow-sm dark:bg-white/[.07] dark:text-white">
      ...
      <section>
        <div className="mb-2 flex items-center justify-between px-1">
          <h2 className="text-[13px] font-semibold text-neutral-900 dark:text-white">Quick access</h2>
          <Link to="/semua-fitur" className="text-[11px] font-semibold text-emerald-700 dark:text-emerald-300">All features ›</Link>
        </div>
        <div className="no-scrollbar -mx-1 flex snap-x gap-3 overflow-x-auto px-1 pb-2">
          {PINTASAN.map((p) => (
            <Link key={p.to + p.label} to={p.to} className="w-[84px] shrink-0 snap-start text-center active:scale-95">
              ...
              <span className="mt-1.5 block truncate text-[10px] font-medium text-neutral-700 dark:text-neutral-200">{p.label}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-[28px] border border-neutral-200 bg-white p-4 shadow-[0_12px_35px_rgba(25,45,55,.06)] dark:border-white/10 dark:bg-white/[.035] sm:p-5">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <div className="text-[10px] font-semibold uppercase tracking-[.14em] text-neutral-400">Widgets</div>
            <h2 className="mt-1 text-[18px] font-semibold tracking-tight text-neutral-950 dark:text-white">My dashboard</h2>
          </div>
          <Link to="/atur-fitur" className="shrink-0 rounded-full bg-neutral-100 px-3 py-2 text-[10px] font-medium text-neutral-700 dark:bg-white/10 dark:text-neutral-200">Customize</Link>
        </div>
      ...
      <details className="group rounded-[26px] border border-neutral-200 bg-white p-4 dark:border-white/10 dark:bg-white/[.035]">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
          <div>
            <div className="text-[10px] font-semibold uppercase tracking-[.14em] text-neutral-400">Today</div>
            <div className="mt-1 text-[16px] font-semibold text-neutral-950 dark:text-white">Log my day or workout</div>
          </div>
          <span className="grid h-9 w-9 place-items-center rounded-full bg-neutral-100 text-lg font-medium text-neutral-700 transition group-open:rotate-45 dark:bg-white/10 dark:text-white">＋</span>
        </summary>
