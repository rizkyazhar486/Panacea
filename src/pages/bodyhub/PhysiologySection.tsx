...
const AnatomyAtlasPlusStandalone = lazy(() => import('./AnatomyAtlasPlusStandalone'))
...
export function PhysiologySection({ onPickSystem }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div id="physiology-system-list" className="space-y-3">
      ...
      <div className="rounded-2xl border border-sky-200/70 bg-sky-50/50 p-3 dark:border-sky-300/10 dark:bg-sky-300/5">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            <div className="text-sm font-black text-ink dark:text-white">Atlas+ · complete anatomy study + Breath Atlas</div>
            <p className="mt-0.5 text-[11px] leading-relaxed text-neutral-500">
              Search curated structures, focus/isolate/compare them in a dedicated Body3D viewport, inspect CT sections,
              switch between function and microanatomy, run structure challenges, and follow a breath from airway mechanics to alveolar exchange.
            </p>
          </div>
          <button
            type="button"
            ...
            className="min-h-[34px] shrink-0 rounded-full bg-brand px-3 text-[10.5px] font-black text-white"
          >
            {atlasOpen ? 'Close Atlas+' : 'Open Atlas+'}
          </button>
        </div>
      </div>

      {atlasOpen && (
        <Suspense fallback={<p className="rounded-xl bg-neutral-50 p-3 text-sm text-neutral-500 dark:bg-white/5">Loading Atlas+ study viewport…</p>}>
          <AnatomyAtlasPlusStandalone />
        </Suspense>
      )}
