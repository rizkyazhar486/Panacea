    ...
    <section data-body-respiratory-gas-exchange="v2" className="overflow-hidden rounded-[28px] border border-cyan-300/10 bg-[linear-gradient(145deg,rgba(34,211,238,.06),rgba(2,6,12,.95)_38%,rgba(16,185,129,.055))] text-white shadow-[0_24px_80px_rgba(0,0,0,.28)]">
      ...
      <div className="grid gap-3 p-3 sm:p-4 2xl:grid-cols-[minmax(0,1.05fr)_minmax(360px,.95fr)]">
        <div className="space-y-3">
          ...
          <article className="rounded-[22px] border border-white/[.08] bg-black/20 p-3.5 sm:p-4">
            <div className="text-[8px] font-black uppercase tracking-[.16em] text-emerald-200/65">Airway → alveolus → capillary</div>
            <div className="mt-3 grid gap-2 sm:grid-cols-3">
              <div className="rounded-2xl border border-cyan-300/10 bg-cyan-300/[.035] p-3">
                <div className="text-[8px] font-black uppercase tracking-wide text-cyan-100/55">Conducting airway</div>
                <div className="mt-2 text-2xl font-black">{outputs.relativeAirwayRadius.toFixed(2)}×</div>
                <div className="mt-1 text-[8px] text-white/28">Relative synthetic radius</div>
              </div>
              <div className="rounded-2xl border border-violet-300/10 bg-violet-300/[.035] p-3">
                <div className="text-[8px] font-black uppercase tracking-wide text-violet-100/55">Alveolar unit</div>
                <div className="mt-2 text-2xl font-black">{respiratoryPercent(outputs.alveolarVentilationSignal)}%</div>
                <div className="mt-1 text-[8px] text-white/28">Normalized ventilation signal</div>
              </div>
              <div className="rounded-2xl border border-emerald-300/10 bg-emerald-300/[.035] p-3">
                <div className="text-[8px] font-black uppercase tracking-wide text-emerald-100/55">Capillary interface</div>
                <div className="mt-2 text-2xl font-black">{respiratoryPercent(outputs.oxygenTransferSignal)}%</div>
                <div className="mt-1 text-[8px] text-white/28">Normalized O₂ transfer signal</div>
              </div>
            </div>
            <div className="mt-3 rounded-2xl border border-amber-300/10 bg-amber-300/[.025] p-3 text-[9px] leading-relaxed text-amber-50/45">
              Dominant teaching constraint: <span className="font-black text-amber-100/75">{outputs.dominantConstraint.replaceAll('-', ' ')}</span>. This is a model-state label, not a disease classification.
            </div>
          </article>
        </div>
        ...
        <aside className="space-y-3">
              ...
              {OUTPUTS.map(([key, label]) => {
                ...
                return (
                  <div key={key} className="rounded-2xl border border-white/[.07] bg-black/20 p-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[8px] font-black text-white/45">{label}</span>
                      <span className="font-mono text-[9px] font-black text-white/70">{respiratoryPercent(value)}%</span>
                    </div>
                    <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/[.05]"><div className="h-full rounded-full bg-white/45" style={{ width: `${respiratoryPercent(value)}%` }} /></div>
                  </div>
                )
              })}
            </div>
          ...
          <article className="rounded-[22px] border border-amber-300/10 bg-amber-300/[.025] p-3.5">
            ...
            <div className="mt-2 space-y-2">
              {RESPIRATORY_TEACHING_EQUATIONS.map((equation) => (
                <div key={equation.expression} className="rounded-2xl border border-white/[.07] bg-black/20 p-3">
                  <div className="text-[8px] font-black text-white/35">{equation.label}</div>
                  <div className="mt-1 break-words font-mono text-[11px] font-black text-white/78">{equation.expression}</div>
                  <p className="mt-1.5 text-[8px] leading-relaxed text-white/28">{equation.note}</p>
                </div>
              ))}
            </div>
          </article>
          ...
          <article className="rounded-[22px] border border-cyan-300/10 bg-cyan-300/[.025] p-3.5">
            ...
            <div className="mt-2 space-y-2">
              {RESPIRATORY_EVIDENCE.map((source) => (
                <a key={source.pmid} href={source.url} target="_blank" rel="noreferrer" className="block rounded-2xl border border-white/[.07] bg-black/20 p-3 hover:border-cyan-300/20">
                  <div className="flex items-center justify-between gap-2"><span className="text-[8px] font-black text-cyan-100/70">PMID {source.pmid}</span><span className="text-[8px] text-white/25">{source.year} ↗</span></div>
                  <div className="mt-1 text-[9px] font-black leading-snug text-white/65">{source.title}</div>
                  <p className="mt-1.5 text-[8px] leading-relaxed text-white/28">{source.role}</p>
                </a>
              ))}
            </div>
          </article>
