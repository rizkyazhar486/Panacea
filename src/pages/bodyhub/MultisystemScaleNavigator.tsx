...
export default function MultisystemScaleNavigator() {
  ...
  return (
    <section data-body-multisystem-scale-navigator="v1" data-selected-scale={selected.scale} className="rounded-2xl border border-neutral-200 bg-white/80 p-3 dark:border-white/10 dark:bg-white/5">
      ...
      <div className="mt-3 rounded-2xl border border-emerald-400/20 bg-emerald-50/60 p-3 dark:bg-emerald-950/20">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <div className="text-[9px] font-black uppercase tracking-[0.16em] text-brand">Respiratory experience</div>
            <div className="mt-1 text-sm font-black text-neutral-950 dark:text-white">Airway → lung → pleura → diaphragm</div>
          </div>
          <button
            type="button"
            aria-expanded={respiratoryOpen}
            onClick={() => setRespiratoryOpen((value) => !value)}
            className="min-h-11 rounded-xl border border-emerald-400/30 px-4 text-[10px] font-black text-brand transition hover:bg-emerald-400/10"
          >
            {respiratoryOpen ? 'Collapse breathing studio' : 'Open breathing studio'}
          </button>
        </div>
      </div>
