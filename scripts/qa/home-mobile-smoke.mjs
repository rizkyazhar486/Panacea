...
async function viewportHealth() {
  return page.evaluate(() => ({ width: window.innerWidth, height: window.innerHeight, scrollWidth: document.documentElement.scrollWidth, bodyScrollWidth: document.body.scrollWidth }))
}
...
try {
  ...
  try {
    ...
    await page.getByText(/Add your first health or daily entry/i).first().waitFor({ state: 'visible' })
    assert(await page.locator('[aria-label="Recorded health signals"]').count() === 0, 'Cross-tab removal must clear the external Home daily snapshot')
  } finally {
    await peer.close()
  }

  const touchStyles = await page.locator('.home-odyssey-hero').evaluate((node) => ({ backdropFilter: getComputedStyle(node).backdropFilter, webkitBackdropFilter: getComputedStyle(node).webkitBackdropFilter }))
  ...
  console.log('Home mobile 390x844 smoke: empty state is honest, real cross-tab daily state adoption works without reload, cross-tab removal clears the recorded-signal rail, touch compositor guard is active, 3D remains opt-in, provenance survives reload, and no horizontal overflow was detected.')
} finally {
  await browser.close()
}
