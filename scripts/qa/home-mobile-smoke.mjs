...
try {
  ...
  const emptySignal = page.getByText(/Add your first health or daily entry/i).first()
  await emptySignal.waitFor({ state: 'visible' })
  ...
  assert(
    touchStyles.backdropFilter === 'none' || touchStyles.webkitBackdropFilter === 'none',
    `Touch Home must disable heavy backdrop filters; got ${JSON.stringify(touchStyles)}`,
  )
  ...
  await page.evaluate(() => {
    localStorage.setItem('pmd_vitals_v1', JSON.stringify({
      steps: 4321,
      source: 'Home QA recorded source',
  ...
  console.log('Home mobile 390x844 smoke: empty state is honest, touch compositor guard is active, 3D remains opt-in, recorded provenance survives reload, and no horizontal overflow was detected.')
} finally {
  await browser.close()
}
