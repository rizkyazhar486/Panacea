...
await context.addInitScript(() => {
  const account = {
    email: 'panacea-os-qa@localhost.test',
    name: 'Panacea OS QA',
    role: 'pasien',
    isSubscriber: false,
...
try {
  ...
  async function addItem(clock, domain, text) {
    await time.fill(clock)
    await category.selectOption(domain)
    await title.fill(text)
    await add.click()
    await rail.getByText(text, { exact: true }).first().waitFor({ state: 'visible', timeout: 5_000 })
  }

  // Enter deliberately out of order; UI/persistence must normalize by time.
  ...
  const storage = await page.evaluate(() => {
    ...
    try {
      ...
      return { key, count: Array.isArray(value) ? value.length : 0 }
    } catch {
      return { key, count: -1 }
    }
  })
