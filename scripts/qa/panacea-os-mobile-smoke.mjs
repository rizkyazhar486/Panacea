...
try {
  ...
  for (const pattern of [/Get Started/i, /Maybe later/i]) {
  ...
  const category = rail.getByLabel('Category')
  const title = rail.getByLabel('Agenda item')
  ...
  const first = await focusCards.nth(0).innerText(); const second = await focusCards.nth(1).innerText(); const third = await focusCards.nth(2).innerText()
  ...
  const storage = await page.evaluate(() => {
    const key = Object.keys(localStorage).find((candidate) => candidate.startsWith('panacea.os.day.v1:'))
    if (!key) return { key: null, count: 0 }
    try { const value = JSON.parse(localStorage.getItem(key) || '[]'); return { key, count: Array.isArray(value) ? value.length : 0 } } catch { return { key, count: -1 } }
  })
  ...
  await rail.scrollIntoViewIfNeeded(); await page.screenshot({ path: screenshotPath, fullPage: false })
  await writeFile(metricsPath, JSON.stringify(metrics, null, 2)); console.log(JSON.stringify(metrics, null, 2))
} catch (error) {
