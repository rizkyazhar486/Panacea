...
await context.addInitScript(() => {
  const account = {
    email: 'panacea-os-qa@localhost.test', name: 'Panacea OS QA', role: 'pasien', isSubscriber: false,
    loggedAt: new Date().toISOString(), sex: 'P', dob: '1990-01-01',
  }
...
try {
  ...
  const first = await focusCards.nth(0).innerText()
  const second = await focusCards.nth(1).innerText()
  const third = await focusCards.nth(2).innerText()
  ...
  const storage = await page.evaluate(() => {
    ...
    try { const value = JSON.parse(localStorage.getItem(key) || '[]'); return { key, count: Array.isArray(value) ? value.length : 0 } }
    catch { return { key, count: -1 } }
  })
