...
test('mobile shell keeps FAB as the single persistent navigation layer', () => {
...
test('consolidation is presentation-only and preserves navigation source', () => {
  assert.match(shell, /quick\.map/)
  assert.match(shell, /<DrawerNav items=\{items\}/)
  assert.match(fabPolicy, /KATALOG_AKSI/)
})
