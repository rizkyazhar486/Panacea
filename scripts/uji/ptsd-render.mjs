...
if (kotak) { await kotak.fill('Post traumatic'); await p.waitForTimeout(1000) }
const t = await p.evaluate(() => document.body.innerText)
console.log('halaman memuat "post traumatic stress disorder":', /post traumatic stress disorder/i.test(t))
...
const dibuka = await p.evaluate(() => {
  for (const el of document.querySelectorAll('a,button,li,div[role="button"]')) {
    if (/post traumatic stress disorder/i.test((el.textContent || '').trim()) && el.getBoundingClientRect().height > 0) { el.click(); return (el.textContent||'').trim().slice(0,50) }
  }
  return null
})
...
for (const w of ['sertralin', 'fluoksetin', 'BENZODIAZEPIN TIDAK DIANJURKAN', 'prazosin', 'AMIGDALA', 'reaksi stres akut', 'bunuh diri', '4 SAMPAI 6 MINGGU'])
