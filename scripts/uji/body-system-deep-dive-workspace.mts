...
const respiratory = readFileSync(resolve('src/pages/bodyhub/RespiratoryGasExchangeWorkbench.tsx'), 'utf8')

assert.match(deep, /cardiovascular:/)
assert.match(deep, /nervous:/)
assert.match(deep, /respiratory:/)
...
assert.match(deep, /selectedAtlasSystemId === 'respiratory'/)
assert.match(deep, /selected-system only/i)
assert.match(deep, /NeurovascularPerfusionWorkbench/)
assert.match(deep, /RespiratoryGasExchangeWorkbench/)
...
assert.doesNotMatch(respiratory, /selectedAtlasSystemId/, 'respiratory workbench context must be owned by the shared selected-system workspace')
...
assert.doesNotMatch(os, /const RespiratoryGasExchangeWorkbench = lazy/)

console.log('body system deep dive: cardiovascular, nervous and respiratory labs are independently context-locked between physiology orientation and failure mechanisms')
