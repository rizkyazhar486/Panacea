...
if (kotak) { await kotak.fill('Skizofrenia'); await p.waitForTimeout(1000) }
const t = await p.evaluate(() => document.body.innerText)
console.log('halaman memuat "skizofrenia":', /skizofrenia/i.test(t))
...
const dibuka = await p.evaluate(() => {
  for (const el of document.querySelectorAll('a,button,li,div[role="button"]')) {
    if (/skizofrenia/i.test((el.textContent || '').trim()) && el.getBoundingClientRect().height > 0) { el.click(); return (el.textContent||'').trim().slice(0,50) }
  }
  return null
})
...
for (const w of ['risperidon', 'haloperidol', 'triheksifenidil', 'AKATISIA', 'MESOLIMBIK', 'DELIRIUM', 'PEMASUNGAN TIDAK DIBENARKAN', 'diskinesia tardif'])
