...
for (const structure of [
  ...
  assert.ok(names.includes(structure), `thyroid/parathyroid atlas must expose ${structure}`)
}
...
assert.match(specialty, /Endocrine.*\['endokrin', 'tiroid'\]/, 'thyroid/parathyroid module must stay reachable from the Endocrine group')
...
assert.match(specialty, /Thyroid gland, all four parathyroids/, 'thyroid atlas must disclose its bounded gland and airway context')
