...
assert.ok(bounded)
assert.equal(bounded.observedDays, MAX_NUTRITION_TREND_DAYS, 'Trend must retain at most the latest bounded recorded days.')
...
assert.equal(withMalformed?.latestDate, '2026-09-05', 'Malformed observations must fail closed through the journal sanitizer.')
