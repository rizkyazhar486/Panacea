...
assert.equal(byId.get('angle-closure-pupillary-block')?.evidence[0]?.citation, 'Laser therapy and angle-closure glaucoma.')
assert.equal(byId.get('angle-closure-pupillary-block')?.evidence[1]?.citation, 'Angle-closure glaucoma: the role of the lens in the pathogenesis, prevention, and treatment.')
