...
if (kotak) { await kotak.fill('Carpal tunnel'); await p.waitForTimeout(1000) }
const t = await p.evaluate(() => document.body.innerText)
console.log('halaman memuat "carpal tunnel syndrome":', /carpal tunnel syndrome/i.test(t))
...
const dibuka = await p.evaluate(() => {
  for (const el of document.querySelectorAll('a,button,li,div[role="button"]')) {
    if (/carpal tunnel syndrome/i.test((el.textContent || '').trim()) && el.getBoundingClientRect().height > 0) { el.click(); return (el.textContent||'').trim().slice(0,50) }
  }
  return null
})
...
for (const w of ['ATROFI OTOT TENAR', 'metilprednisolon', 'Phalen', 'ligamentum karpal', 'RADIKULOPATI SERVIKAL', 'KEHAMILAN', 'JANGAN MENUNDA RUJUKAN BEDAH'])
