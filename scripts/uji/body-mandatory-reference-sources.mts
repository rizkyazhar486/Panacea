...
// A visual reference is not automatically a licensed or academically verified
// geometry source. Keep these fail-closed until exact asset-level evidence exists.
assert.equal(BODY_MANDATORY_REFERENCE_SOURCES.every((source) => source.licenseStatus === 'pending'), true)

console.log('Body mandatory references: thebuggeddev anatomy + Breath Atlas pinned with 5K reference target and fail-closed asset licensing.')
