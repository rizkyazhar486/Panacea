...
import { PANCREAS_EDUCATION_EDGES, PANCREAS_EDUCATION_NODES, PANCREAS_SYSTEM_ID, validatePancreasEducationGraph } from '../../src/lib/bodyPancreasEducation.ts'

assert.equal(PANCREAS_SYSTEM_ID, 'endocrine')
...
assert.match(PANCREAS_EDUCATION_NODES.find((node) => node.kind === 'physiology')!.boundary, /no measured secretion/i)
assert.match(PANCREAS_EDUCATION_NODES.find((node) => node.kind === 'pharmacology')!.boundary, /no drug selection/i)
assert.match(PANCREAS_EDUCATION_NODES.find((node) => node.kind === 'imaging')!.boundary, /no ultrasound/i)
