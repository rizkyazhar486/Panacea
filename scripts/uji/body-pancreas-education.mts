...
import {
  PANCREAS_EDUCATION_EDGES,
  PANCREAS_EDUCATION_NODES,
  PANCREAS_SYSTEM_ID,
  validatePancreasEducationGraph,
} from '../../src/lib/bodyPancreasEducation.ts'

assert.equal(PANCREAS_SYSTEM_ID, 'digestive')
for (const kind of ['anatomy', 'physiology', 'pathophysiology', 'pharmacology', 'imaging'] as const) {
  assert.ok(PANCREAS_EDUCATION_NODES.some((node) => node.kind === kind))
}
...
for (const node of PANCREAS_EDUCATION_NODES) {
  assert.equal(node.evidenceState, 'educational-only')
  assert.ok(node.boundary.length > 40)
}
for (const edge of PANCREAS_EDUCATION_EDGES) {
...
assert.match(PANCREAS_EDUCATION_NODES.find((node) => node.kind === 'physiology')!.boundary, /no endocrine secretion/i)
assert.match(PANCREAS_EDUCATION_NODES.find((node) => node.kind === 'pharmacology')!.boundary, /no drug selection/i)
assert.match(PANCREAS_EDUCATION_NODES.find((node) => node.kind === 'imaging')!.boundary, /no ultrasound, CT, MRI/i)
