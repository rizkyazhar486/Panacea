...
assert.ok(SMART_NOTIFICATION_RULES.length >= 30, 'smart notification engine should expose at least 30 useful combinations')
...
console.log(`smart notifications: ${SMART_NOTIFICATION_RULES.length} rules, ${defaultEnabledRuleIds().length} enabled by default; all assertions passed`)
