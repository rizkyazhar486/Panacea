...
assert.equal(
  new Set(BODY_PHARMACOLOGY_MECHANISM_NETWORK.map((mechanism) => mechanism.id)).size,
  BODY_PHARMACOLOGY_MECHANISM_NETWORK.length,
  'pharmacology mechanism ids must be unique',
)
...
for (const mechanism of BODY_PHARMACOLOGY_MECHANISM_NETWORK) {
  ...
  assert.deepEqual(
    mechanism.mechanismChain.map((step) => step.layer),
    ['target', 'molecular', 'cellular', 'organ', 'systems'],
    `${mechanism.id} must preserve the five-scale mechanism order`,
  )
...
assert.deepEqual(
  getBodyPharmacologyMechanism('statin-hmgcr').mechanismChain.map((step) => step.layer),
  ['target', 'molecular', 'cellular', 'organ', 'systems'],
  'statin mechanism must preserve scale continuity',
)
assert.ok(
  getBodyPharmacologyMechanism('aspirin-cox1').mechanismChain.some((step) => /thromboxane/i.test(step.mechanism)),
  'aspirin mechanism must retain thromboxane biology',
)
assert.ok(
  getBodyPharmacologyMechanism('ace-inhibition').linkedScenarioIds.includes('heart-failure'),
  'ACE inhibition must connect to the heart-failure mechanism network',
)
assert.ok(
  getBodyPharmacologyMechanism('sglt2-inhibition').physiologySystemIds.includes('renal'),
  'SGLT2 inhibition must preserve its canonical renal target context',
)
assert.ok(
  getBodyPharmacologyMechanism('factor-xa-inhibition').linkedScenarioIds.includes('venous-thromboembolism'),
  'factor Xa inhibition must connect to VTE pathophysiology',
)
assert.ok(
  getBodyPharmacologyMechanism('pcsk9-inhibition').mechanismChain.some((step) => /LDL-receptor degradation decreases/i.test(step.label)),
  'PCSK9 mechanism must preserve LDLR-degradation biology',
)
