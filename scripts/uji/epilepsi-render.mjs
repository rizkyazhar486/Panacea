...
if (kotak) { await kotak.fill('Epilepsi'); await p.waitForTimeout(1000) }
const t = await p.evaluate(() => document.body.innerText)
console.log('halaman memuat "epilepsi":', /epilepsi/i.test(t))
...
const dibuka = await p.evaluate((nama) => {
  ...
  // katanya. Percobaan pertama memakai /epilepsi/i dan mendarat pada entri
  // "Kejang" yang kebetulan menyebut epilepsi di dalam catatannya — lalu
...
for (const w of ['STATUS EPILEPTIKUS', 'levetirasetam', 'asam valproat', 'BUKAN penyakit menular', 'kelumpuhan Todd', 'BANGKITAN TERPROVOKASI'])
