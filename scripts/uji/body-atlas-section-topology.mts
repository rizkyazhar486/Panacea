...
const truncatedTopology = classifyBodyAtlasSectionTopology(exact.segments, { sourceSectionTruncated: true })
assert.equal(truncatedTopology.sourceSectionTruncated, true)
assert.ok(truncatedTopology.components.every((component) => component.completeSourceSection === false))
assert.ok(truncatedTopology.components.every((component) => component.publishableContour === false))
