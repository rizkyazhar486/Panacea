...
for (const guard of [
  'third-party-ux-reference-to-medical-evidence',
  'unknown-license-reference-to-runtime-asset',
  'gene-or-rna-to-deterministic-thought',
  'hormone-to-deterministic-personality',
  'atlas-geometry-to-patient-specific-anatomy',
  'longevity-research-to-immortality-claim',
]) {
  assert.ok(data.forbiddenPromotions.includes(guard), `${guard} must remain fail-closed`)
}

console.log('Multisystem evidence boundary verified: mandatory external references remain reference-only, Japanese iPSC evidence is pinned, and longevity research cannot be promoted to an immortality claim.')
