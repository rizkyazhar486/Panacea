...
if (kotak) { await kotak.fill('Glomerulonefritis'); await p.waitForTimeout(1000) }
const t = await p.evaluate(() => document.body.innerText)
console.log('halaman memuat "Glomerulonefritis akut":', /glomerulonefritis akut/i.test(t))
...
const dibuka = await p.evaluate(() => {
  for (const el of document.querySelectorAll('a,button,li,div[role="button"]')) {
    if (/glomerulonefritis akut/i.test((el.textContent || '').trim()) && el.getBoundingClientRect().height > 0) { el.click(); return (el.textContent||'').trim().slice(0,50) }
  }
  return null
})
...
for (const w of ['SINDROM NEFRITIK', 'C3', 'penisilin', 'benzatin', 'furosemid', 'nifedipin', 'eritromisin', 'ensefalopati hipertensif', 'ASTO', 'Tatalaksana|TATALAKSANA'])
