...
assert.ok(MULTISYSTEM_DOMAINS.length >= 15, 'graph should cover major organ systems plus cross-cutting molecular/cognition/regeneration/aging domains')

for (const scale of requiredScales) {
  assert.ok(getDomainsForScale(scale).length > 0, `${scale} must have at least one represented domain`)
}
...
for (const id of [
  ...
  'cell-molecular-genomics',
  'brain-cognition',
  'stem-cell-regeneration',
  'aging-longevity',
]) {
  assert.ok(getMultisystemDomain(id), `${id} must remain in the canonical multisystem graph`)
}
...
console.log('Multisystem end-to-end knowledge graph verified: whole body → organ → cell/organelle → pathways/protein/RNA/DNA → neural/endocrine/cognition → regeneration → aging, with mandatory reference and evidence boundaries intact.')
