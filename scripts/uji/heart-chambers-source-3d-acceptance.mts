...
for (const chamber of ['left cardiac atrium', 'right cardiac atrium', 'left ventricle', 'right ventricle']) assert.ok(names.includes(chamber), `heart atlas must expose ${chamber}`)
assert.ok(names.includes('interventricular septum'), 'heart atlas must expose the interventricular septum')
for (const valve of ['aortic valve', 'pulmonary valve', 'mitral valve', 'tricuspid valve']) assert.ok(names.includes(valve), `heart atlas must expose ${valve}`)
