...
assert.equal(audit.duplicateIds, false); assert.deepEqual(audit.danglingEdges, []); assert.deepEqual(audit.nonEducationalNodes, []); assert.deepEqual(audit.boundaryMissing, [])
