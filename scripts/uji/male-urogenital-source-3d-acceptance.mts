...
for (const structure of ['corpus cavernosum of penis','corpus spongiosum of penis','glans penis','left deferent duct','right deferent duct','left epididymis','right epididymis','left seminal vesicle','right seminal vesicle','left testis','right testis','left kidney','right kidney','left ureter','right ureter','urethra','urinary bladder']) assert.ok(names.includes(structure), `male urogenital atlas must expose ${structure}`)
assert.ok(names.includes('prostate'), 'male urogenital atlas must retain source-backed prostate context without replacing dedicated prostate-zone module')
...
assert.match(specialty, /Urogenital.*\['urogenital', 'prostat'\]/, 'male urogenital module must stay reachable from Urogenital')
