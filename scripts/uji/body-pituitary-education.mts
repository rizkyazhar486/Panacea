...
import {
  PITUITARY_EDUCATION_EDGES,
  PITUITARY_EDUCATION_NODES,
  PITUITARY_SYSTEM_ID,
  validatePituitaryEducationGraph,
} from '../../src/lib/bodyPituitaryEducation.ts'
...
for (const kind of ['anatomy', 'physiology', 'pathophysiology', 'pharmacology', 'imaging'] as const) {
  assert.ok(PITUITARY_EDUCATION_NODES.some((node) => node.kind === kind))
}
...
for (const node of PITUITARY_EDUCATION_NODES) {
  assert.equal(node.evidenceState, 'educational-only')
  assert.ok(node.boundary.length > 40)
}
for (const edge of PITUITARY_EDUCATION_EDGES) {
  assert.notEqual(edge.from, edge.to)
  assert.ok(edge.note.length > 40)
}
assert.match(PITUITARY_EDUCATION_NODES.find((node) => node.kind === 'anatomy')!.boundary, /no geometry/i)
assert.match(PITUITARY_EDUCATION_NODES.find((node) => node.kind === 'physiology')!.boundary, /no hormone/i)
assert.match(PITUITARY_EDUCATION_NODES.find((node) => node.kind === 'pharmacology')!.boundary, /no drug selection/i)
