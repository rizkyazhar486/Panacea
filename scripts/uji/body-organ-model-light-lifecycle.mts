...
assert.match(source, /disposeObject3D\(scene\)/, 'mounted scene resources must be disposed on unmount')
assert.match(source, /if \(disposed\) \{\s*disposeObject3D\(gltf\.scene\)/s, 'late GLTF success after unmount must dispose the decoded scene')
assert.match(source, /webglcontextlost/, 'context loss must fail closed instead of leaving a frozen canvas')
assert.match(source, /event\.preventDefault\(\)/, 'context loss handling must explicitly own the browser recovery path')
