...
const referencesSource = readFileSync(new URL('../../src/lib/respiratoryReferences.ts', import.meta.url), 'utf8')
...
assert.match(quantitativeSource, /RESPIRATORY_TEACHING_REFERENCES/)
...
assert.match(quantitativeSource, /Physiology references · verify at source/)
...
assert.match(referencesSource, /https:\/\/www\.ncbi\.nlm\.nih\.gov\/books\/NBK482502\//)
assert.match(referencesSource, /https:\/\/www\.ncbi\.nlm\.nih\.gov\/books\/NBK442016\//)
assert.match(referencesSource, /https:\/\/www\.ncbi\.nlm\.nih\.gov\/books\/NBK482501\//)
assert.doesNotMatch(referencesSource, /fetch\s*\(/, 'Teaching references must remain static provenance, not a new runtime dependency.')
...
console.log('Breath Atlas remains source-aware, quantitatively bounded, independently implemented, referenced, and license-gated.')
