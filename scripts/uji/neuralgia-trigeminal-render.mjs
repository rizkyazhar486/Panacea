...
if (kotak) { await kotak.fill('Neuralgia trigeminal'); await p.waitForTimeout(1000) }
const t = await p.evaluate(() => document.body.innerText)
console.log('halaman memuat "neuralgia trigeminal":', /neuralgia trigeminal/i.test(t))
...
const dibuka = await p.evaluate(() => {
  for (const el of document.querySelectorAll('a,button,li,div[role="button"]')) {
    if (/neuralgia trigeminal/i.test((el.textContent || '').trim()) && el.getBoundingClientRect().height > 0) { el.click(); return (el.textContent||'').trim().slice(0,50) }
  }
  return null
})
...
for (const w of ['KARBAMAZEPIN', 'okskarbazepin', 'HLA-B', 'DEKOMPRESI MIKROVASKULAR', 'DEMIELINISASI', 'BUKAN PENYAKIT GIGI', 'dua menit'])
