...
import {
  MULTISCALE_SOURCE_POLICY,
  requiredSourceContextForScale,
  sourcePolicyForScale,
  validateEvidenceSourceForScale,
} from '../../src/lib/bodyMultiscaleSourcePolicy.ts'
...
const pdbProtein = validateEvidenceSourceForScale('protein', {
  sourceId: 'rcsb-pdb',
  sourceVersion: '2026-09-09-entry-revision',
  locator: 'PDB:1ABC/entity:1/assembly:1',
  citation: 'Fixture PDB citation; deterministic test only.',
})
assert.equal(pdbProtein.valid, true, pdbProtein.reasons.join(' | '))

const hpaOrganelle = validateEvidenceSourceForScale('organelle', {
  sourceId: 'human_protein_atlas',
  sourceVersion: '25.1',
  locator: 'ENSG00000000000/subcellular-location',
  citation: 'Fixture HPA citation; deterministic test only.',
})
assert.equal(hpaOrganelle.valid, true, hpaOrganelle.reasons.join(' | '))

const wrongGrossSource = validateEvidenceSourceForScale('organ', {
  sourceId: 'reactome',
  sourceVersion: 'release-99',
  locator: 'R-HSA-000000',
  citation: 'Fixture only.',
})
...
const wrongGeneSource = validateEvidenceSourceForScale('gene', {
  sourceId: 'rcsb-pdb',
...
for (const forbidden of [
  'patient-specific anatomy confirmed',
  'pathway proves diagnosis',
  'vep proves pathogenic',
  'expression proves treatment',
]) {
  assert.equal(corpus.includes(forbidden), false)
}
