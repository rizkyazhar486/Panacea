...
assert.match(precisionLab, /type Mode = 'holistic' \| 'z-anatomy' \| 'unfolded' \| 'specialty' \| 'movement'/, 'Holistic mode and every existing precision-atlas mode must remain available alongside Z-Anatomy.')
assert.match(precisionLab, /useState<Mode>\('holistic'\)/, 'Holistic healthcare must be the first visible precision-atlas experience.')
assert.match(precisionLab, /\['holistic', 'Holistic healthcare'\]/, 'Holistic healthcare must have an explicit navigation tab.')
assert.match(precisionLab, /\['z-anatomy', 'Z-Anatomy atlas'\]/, 'Z-Anatomy must retain an explicit navigation tab.')
assert.match(precisionLab, /mode === 'z-anatomy'/, 'Existing Z-Anatomy workbench must remain directly reachable.')
...
console.log('Body Exposure Z-Anatomy workbench preserves credited source bundles, provenance, shared-viewer interaction, holistic integration, and scientific boundaries.')
