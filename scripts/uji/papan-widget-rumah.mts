...
assert.ok(!/role="tab"/.test(blokTitik) && !/aria-selected/.test(blokTitik),
  'the dots are marked as tabs again; panacea-control-grading-v46 then paints every selected control as a white neumorphic capsule')
...
assert.deepEqual(jendelaTitik(0, 3), [0, 1, 2])
assert.deepEqual(jendelaTitik(0, 23), [0, 1, 2, 3, 4, 5, 6])
assert.deepEqual(jendelaTitik(11, 23), [8, 9, 10, 11, 12, 13, 14])
assert.deepEqual(jendelaTitik(22, 23), [16, 17, 18, 19, 20, 21, 22])
for (const [aktif, jumlah] of [[0, 1], [5, 9], [40, 41]] as [number, number][]) {
  const w = jendelaTitik(aktif, jumlah)
  assert.ok(w.every((i) => i >= 0 && i < jumlah))
  assert.ok(w.includes(Math.min(aktif, jumlah - 1)))
}
