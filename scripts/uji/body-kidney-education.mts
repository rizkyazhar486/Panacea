...
import { KIDNEY_EDUCATION_EDGES, KIDNEY_EDUCATION_NODES, KIDNEY_SYSTEM_ID, validateKidneyEducationGraph } from '../../src/lib/bodyKidneyEducation.ts'

assert.equal(KIDNEY_SYSTEM_ID, 'urinary')
for (const kind of ['anatomy', 'physiology', 'pathophysiology', 'pharmacology', 'imaging'] as const) assert.ok(KIDNEY_EDUCATION_NODES.some((node) => node.kind === kind))
...
for (const node of KIDNEY_EDUCATION_NODES) { assert.equal(node.evidenceState, 'educational-only'); assert.ok(node.boundary.length > 40) }
for (const edge of KIDNEY_EDUCATION_EDGES) { assert.notEqual(edge.from, edge.to); assert.ok(edge.note.length > 40) }
