...
assert.match(body3d, /Source anatomy is evidence-bearing geometry/, 'renderer must document the non-deforming source-anatomy boundary')
...
assert.match(body3d, /const pixelRatioCap = window\.matchMedia\('\(max-width: 640px\)'\)\.matches \? 1\.5 : 2/, 'mobile pixel ratio must stay more conservative than desktop for GPU stability')
...
console.log(`Body Exposure: Claude shared-body workspace preserved; ${HEAD_TO_TOE_REGIONS.length} ordered regions and ${HEAD_TO_TOE_STRUCTURES.length} named catalogue structures retained with a non-deforming physiology boundary`)