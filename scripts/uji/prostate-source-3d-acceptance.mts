...
for (const required of ['transition', 'central', 'peripheral']) assert.ok(names.some((name) => name.includes(required)), `${required} prostate zone source geometry must be present`)
...
console.log(`Prostate source 3D gate: ${parts.length} exact structures, ${triangles.toLocaleString()} indexed triangles, source=${sources.join(',')}`)
