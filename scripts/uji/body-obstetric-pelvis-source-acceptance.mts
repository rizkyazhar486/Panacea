...
assert.deepEqual(audit.fabricatedFemaleReproductive, [], `obstetri: female reproductive geometry substituted: ${audit.fabricatedFemaleReproductive.join(', ')}`)
...
for (const name of FEMALE_REPRODUCTIVE_NAMES_NOT_SHIPPED_IN_OBSTETRI) assert.ok(!obstetriNames.has(name), `obstetri must not claim female reproductive mesh: ${name}`)
...
console.log('obstetric pelvis source acceptance:', { module: audit.module, structures: audit.structures, triangles: audit.triangles, sources: audit.sources, referenceBody: audit.referenceBody })
