import assert from 'node:assert/strict'
import { readFile } from 'node:fs/promises'
import { ATLAS_MODULE_INFO, partsForModule } from '../../src/lib/systemAtlas.gen'

const specialty = await readFile(new URL('../../src/pages/bodyhub/SpecialtyLab.tsx', import.meta.url), 'utf8')
const bodyExplorer = await readFile(new URL('../../src/pages/BodyExplorer.tsx', import.meta.url), 'utf8')
const viewer = await readFile(new URL('../../src/components/AtlasViewer3D.tsx', import.meta.url), 'utf8')

const moduleId = 'bilier'
const info = ATLAS_MODULE_INFO[moduleId]
const parts = partsForModule(moduleId)
const names = parts.map((part) => part.name.toLowerCase())
const sources = [...new Set(parts.map((part) => part.source))]
const triangles = parts.reduce((sum, part) => sum + Math.max(0, part.triangles), 0)

assert.ok(info, 'biliary/pancreas atlas module must be generated')
assert.ok(parts.length >= 20, 'biliary/pancreas module must preserve broad source-backed ductal and organ coverage')
assert.ok(parts.every((part) => Number.isFinite(part.triangles) && part.triangles > 0), 'every biliary/pancreas structure must have positive indexed source geometry')
assert.ok(triangles > 0, 'biliary/pancreas module must expose positive triangle geometry')
assert.deepEqual(sources, ['hra-female'], 'biliary/pancreas geometry must retain HuBMAP HRA female provenance')

for (const structure of ['gallbladder', 'common bile duct', 'cystic duct', 'common hepatic duct', 'left hepatic duct', 'right hepatic duct', 'hepatopancreatic ampulla']) {
  assert.ok(names.includes(structure), `biliary atlas must expose ${structure}`)
}
assert.ok(names.some((name) => name.includes('dorsal pancreatic duct')), 'biliary atlas must expose the dorsal pancreatic duct')
assert.ok(names.some((name) => name.includes('ventral pancreatic duct')), 'biliary atlas must expose the ventral pancreatic duct')
for (const structure of ['head of pancreas', 'body of pancreas', 'tail of pancreas', 'neck of pancreas', 'uncinate process of pancreas']) {
  assert.ok(names.includes(structure), `biliary/pancreas atlas must expose ${structure}`)
}
assert.ok(names.some((name) => name.includes('liver')), 'biliary atlas must preserve source-backed liver context')

assert.match(bodyExplorer, /const SpecialtyLab = lazy/, 'Body Exposure must keep the specialty atlas reachable')
assert.match(bodyExplorer, /key: 'spesialisasi'/, 'Body Exposure must keep the specialty navigation entry reachable')
assert.match(specialty, /Abdomen.*\['gastro', 'bilier', 'nefrologi'\]/, 'biliary module must stay reachable from the Abdomen atlas group')
assert.match(specialty, /berkas=\{`atlas\/\$\{modul\}\.glb`\}/, 'specialty atlas must load the shipped module GLB')
assert.match(specialty, /partsForModule\(modul\)/, 'specialty atlas must use generated source metadata for selectable geometry')
assert.match(specialty, /SUMBER\[asal\]/, 'specialty atlas must visibly expose source provenance')
assert.match(specialty, /Hepatic ducts, cystic duct, common bile duct/, 'biliary atlas must disclose its bounded ductal scope')
assert.match(viewer, /body3dPixelRatio/, 'viewer must keep bounded mobile pixel-ratio behavior')
assert.match(viewer, /IntersectionObserver/, 'viewer must suspend rendering when offscreen')
assert.match(viewer, /visibilitychange/, 'viewer must suspend rendering while the document is hidden')
assert.match(viewer, /Hanya mesh dengan metadata terverifikasi/, 'viewer must limit picking to verified metadata-backed meshes')
assert.doesNotMatch(specialty, /SphereGeometry|BoxGeometry|CylinderGeometry/, 'biliary acceptance must not depend on synthetic primitive anatomy')

console.log(`biliary/pancreas source 3D acceptance passed: ${parts.length} structures, ${triangles} triangles, source=${sources.join(',')}`)
