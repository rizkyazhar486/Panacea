...
const instruction = namedBox('HOW TO USE THIS MODEL', 0, 100, 0, 10, 10, 10)
...
assert.equal(spatialIndex.nodes.length, 5)
...
assert.ok(airwayTree.size[0] > 1, 'Named parent must receive the union of descendant source geometry.')
...
const crossSection = queryBodyAtlasCrossSection({
  axis: 'x',
...
for (const node of spatialNodes) {
  const object = root.getObjectByProperty('userData.originalName', node.name)
  if (object instanceof THREE.Object3D) object.traverse((child) => {
...
console.log('Body spatial registry: bottom-up source AABBs, broad-phase axial/coronal/sagittal candidates, graph binding, nearest-neighborhood and camera-fit geometry verified.')
