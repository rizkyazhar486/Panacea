...
import {
  GALLBLADDER_EDUCATION_EDGES,
  GALLBLADDER_EDUCATION_NODES,
  GALLBLADDER_SYSTEM_ID,
  validateGallbladderEducationGraph,
} from '../../src/lib/bodyGallbladderEducation.ts'
...
for (const kind of ['anatomy', 'physiology', 'pathophysiology', 'pharmacology', 'imaging'] as const) {
  assert.ok(GALLBLADDER_EDUCATION_NODES.some((node) => node.kind === kind))
}
...
for (const node of GALLBLADDER_EDUCATION_NODES) {
  assert.equal(node.evidenceState, 'educational-only')
  assert.ok(node.boundary.length > 40)
}
for (const edge of GALLBLADDER_EDUCATION_EDGES) {
...
assert.match(GALLBLADDER_EDUCATION_NODES.find((node) => node.kind === 'physiology')!.boundary, /no pressure/i)
