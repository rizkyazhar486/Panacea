...
for (const drug of DRUG_TARGETS) {
  ...
  ok(`${drug.id}: status curated`, mapped.status === 'verified-curated', mapped.status)
...
ok('obat tak dikenal gagal tertutup',
  unknown.status === 'unmapped' && unknown.desiredEffectOrganKeys.length === 0 && unknown.adverseEffectOrganKeys.length === 0)
