...
for (const relationship of ORGAN_EDUCATION_RELATIONSHIPS) {
  ...
  for (const evidenceId of relationship.evidenceIds) assert.ok(evidenceIds.has(evidenceId), `${relationship.id} references missing evidence ${evidenceId}`)
...
assert.equal(getOrganEducationRelationships('thyroid').length, 1)
assert.equal(hasSourceBackedOrganRelationship('pancreas-acinar-duct-exocrine-flow'), true)
assert.equal(hasSourceBackedOrganRelationship('thyroid-follicle-hormone-synthesis'), true)
