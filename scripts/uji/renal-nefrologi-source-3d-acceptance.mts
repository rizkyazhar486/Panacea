...
assert.ok(info, 'renal/urinary module must be shipped')
assert.ok(info.structures >= 14, 'renal/urinary module must preserve generated source breadth')
assert.ok(parts.length >= 14, 'renal/urinary module must expose bilateral organs, collecting tract and vascular context')
assert.ok(parts.every((part) => Number.isFinite(part.triangles) && part.triangles > 0), 'every renal-module structure must carry positive indexed triangles')
assert.ok(triangles > 0, 'renal module must contain indexed source geometry')
assert.deepEqual(sources, ['bodyparts3d'], 'renal geometry must retain BodyParts3D source identity')
...
assert.ok(names.includes('left adrenal gland') && names.includes('right adrenal gland'), 'adjacent bilateral adrenal context must remain present without being relabeled as urinary anatomy')
...
assert.match(specialty, /Abdomen.*\['gastro', 'bilier', 'nefrologi'\]/, 'renal module must remain reachable from Abdomen selector')
...
assert.doesNotMatch(specialty, /SphereGeometry|BoxGeometry|CylinderGeometry/, 'specialty atlas must not synthesize substitute renal geometry')

console.log(`Renal source 3D gate: ${parts.length} exact structures, ${triangles.toLocaleString()} indexed triangles, source=${sources.join(',')}, bilateral kidneys/ureters + bladder/urethra + renal vessels, reachable GLB WebGL and mobile lifecycle locked.`)
